-- =====================================================================
-- Fix E002 — sanitiza campos de texto livre (campaign/ad_group/targeting/
...
-- Marker: E002_SANITIZE_FREETEXT_CSV
-- =====================================================================
UPDATE query_templates
SET version = 2, updated_at = NOW(),
    description = COALESCE(description,'') || ' [sanitize free-text CSV E002_SANITIZE_FREETEXT_CSV]',
    sql_template = $SAN$-- E002_TARGET_DAILY_EXTRACT_V2
-- MarketCloud ZANOM
-- Grão: data_date / campaign_id / ad_product_type / targeting / match_type
-- V2: removido marketplace_id (non-viewable nesta instância AMC)
...
FROM joined
$SAN$
WHERE code = 'MC_ZANOM_E002' AND status = 'ACTIVE';
