-- 200_calibration_expose_blend_internals.sql
-- TRANSPARENCIA DO BLEND ML x DAYPARTING (sem mexer na logica de dinheiro).
-- Ate a 170 o cerebro (kw_roas cru, prior_roas, ml_f, peso w) so existia
-- enterrado no texto do 'reason'. Aqui promovemos as 4 entranhas a COLUNAS de
-- primeira classe, para a tela mostrar, celula a celula, como a decisao nasce:
--   hour_roas (=blended) = blend_weight*kw_roas_raw + (1-blend_weight)*(prior_roas*ml_factor)
-- Aditivo e reversivel: colunas nullable + a MESMA funcao 170 populando-as.
ALTER TABLE marketcloud_gold.gold_keyword_hourly_calibration_v1
  ADD COLUMN IF NOT EXISTS kw_roas_raw  numeric,  -- ROAS real observado da kw x hora (intocado pelo ML)
  ADD COLUMN IF NOT EXISTS prior_roas   numeric,  -- prior pooled (campanha/global) antes do ML
  ADD COLUMN IF NOT EXISTS ml_factor    numeric,  -- nudge do ML (cp_hora/media_cp_kw), clamp [0.6,1.4]
  ADD COLUMN IF NOT EXISTS blend_weight numeric;  -- w = cliques/(cliques+k): confianca na observacao

CREATE OR REPLACE FUNCTION marketcloud_gold.refresh_keyword_hourly_calibration(p_window_days integer DEFAULT 28, p_pool_min_clicks integer DEFAULT 12, p_min_spend numeric DEFAULT 5.0, p_shrink_k numeric DEFAULT 15, p_target_roas numeric DEFAULT 3.0)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    INSERT INTO marketcloud_gold.gold_keyword_hourly_calibration_v1 (
        ...
        kw_roas_raw, prior_roas, ml_factor, blend_weight)
    SELECT
        ...
        END,
        baseline_scope,
        round(kw_roas,4), round(prior_roas,4), round(ml_f,4), w
    FROM decided;
