-- 086: o grupo de controle sai das recomendacoes da tela Keywords x hora.
-- O auto-apply ja respeita o holdout (guard no worker). Faltava o caminho
-- humano: o dono clicando Aplicar numa celula de controle mata o experimento
-- do mesmo jeito. O SWARM nao le o MarketCloud (o fdw e de mao unica), entao
-- em vez de fdw reverso a celula simplesmente nao aparece na lista.
...
SELECT v.keyword_hour_recommendation_id,
   ...
   FROM marketcloud_gold.gold_keyword_hourly_recommendations_v2 v
     ...
     JOIN LATERAL ( SELECT b.keyword_id
          ...
          WHERE b.campaign_id = v.campaign_id AND COALESCE(b.ad_group_id, ''::text) = COALESCE(v.ad_group_id, ''::text) AND lower(TRIM(BOTH FROM b.keyword_text)) = lower(TRIM(BOTH FROM v.keyword_text)) AND lower(TRIM(BOTH FROM COALESCE(b.match_type, ''::text))) = lower(TRIM(BOTH FROM COALESCE(v.match_type, ''::text))) AND upper(COALESCE(b.state, ''::text)) = 'ENABLED'::text AND COALESCE(b.keyword_id, ''::text) <> ''::text
         LIMIT 1) k ON true
     LEFT JOIN LATERAL ( SELECT COALESCE(s.multiplier, 1.0) AS multiplier,
          ...
          WHERE s.hour_start <= v.event_hour AND s.hour_end > v.event_hour AND COALESCE(s.day_of_week, ''::text) = ''::text AND COALESCE(s.profile_is_active, true) = true AND upper(COALESCE(s.profile_status, ''::text)) = 'PUBLISHED'::text AND s.multiplier IS NOT NULL AND (s.scope = 'ENTITY'::text AND s.campaign_id = v.campaign_id AND (s.entity_id = k.keyword_id OR COALESCE(s.entity_id, ''::text) = ''::text AND lower(TRIM(BOTH FROM COALESCE(s.entity_label, ''::text))) = lower(TRIM(BOTH FROM v.keyword_text))) OR s.scope = 'AD_GROUP'::text AND s.campaign_id = v.campaign_id AND s.ad_group_id = v.ad_group_id OR s.scope = 'CAMPAIGN'::text AND s.campaign_id = v.campaign_id OR s.scope = 'GLOBAL'::text)
          ORDER BY (
                CASE s.scope
  ...
  -- GRUPO DE CONTROLE: a celula sorteada nao vira recomendacao. E o jeito de
  -- proteger o experimento do lado humano sem precisar de fdw reverso: se a
  -- linha nao aparece, ninguem pina ela por engano.
  AND NOT EXISTS (
