-- 198 — Search Intelligence: excluir pedidos de REMOCAO/DEVOLUCAO FBA (sales_channel=
-- 'Non-Amazon', prefixo S01-, R$0) da contagem de unidades/receita por ASIN. Espelha o
-- fix do dashboard (amazonOrdersWhere) -> reconcilia Dashboard x Search Intelligence.
-- Ex B0H6ZVT5BM: 124 unidades eram devolucao de estoque, nao venda.
CREATE OR REPLACE VIEW marketcloud_gold.gold_search_intelligence_product_daily_v1 AS
 ...
 SELECT k.data_date,
    ...
    COALESCE(oi.gross_sales, 0::numeric) * 0.02 AS coper_estimated,
    COALESCE(oi.gross_sales, 0::numeric) - COALESCE(oi.total_units, 0::numeric) * COALESCE(d.unit_cost, 0::numeric) - COALESCE(oi.total_units, 0::numeric) * COALESCE(f.unit_amazon_fee, 0::numeric) - COALESCE(a.ads_spend, 0::numeric) - COALESCE(oi.gross_sales, 0::numeric) * 0.04 - COALESCE(oi.gross_sales, 0::numeric) * 0.02 AS ebitda_estimated,
        CASE
