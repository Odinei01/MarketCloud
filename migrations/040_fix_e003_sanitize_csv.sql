-- =====================================================================
-- Fix E003 — sanitiza campos de texto livre (campaign/ad_group/targeting/
-- customer_search_term): remove aspas e virgulas que quebram o CSV do AMC.
-- Uma aspa desbalanceada fundia milhares de linhas num campo unico (bug do
-- LazyQuotes), descartando quase todo o extrato. Mesmo fix aplicado ao E005.
-- Marker: E003_SANITIZE_FREETEXT_CSV
-- =====================================================================
UPDATE query_templates
SET version = 2, updated_at = NOW(),
    description = COALESCE(description,'') || ' [sanitize free-text CSV E003_SANITIZE_FREETEXT_CSV]',
    sql_template = $SAN$-- E003_SEARCH_TERM_DAILY_EXTRACT_V1_APPROVED_CORRECTED
-- MarketCloud ZANOM
-- Grão: data_date / campaign_id / ad_product_type / ad_group / targeting / match_type / customer_search_term
-- marketplace_id removido (non-viewable nesta instância AMC)
...
WITH traffic_raw AS (
    ...
    GROUP BY
        ...
        ad_group, targeting, match_type, customer_search_term,
        marketplace_name, currency_iso_code, portfolio_id, portfolio_name
),
...
traffic AS (
    SELECT
        ...
        NULLIF(TRIM(REPLACE(REPLACE(CAST(targeting AS VARCHAR), '"', ' '), ',', ' ')), '') AS targeting,
        COALESCE(NULLIF(TRIM(CAST(match_type AS VARCHAR)), ''), 'NO_MATCH_TYPE') AS match_type,
        NULLIF(TRIM(REPLACE(REPLACE(CAST(customer_search_term AS VARCHAR), '"', ' '), ',', ' ')), '') AS customer_search_term,
...
conversions_raw AS (
    ...
    WHERE conversion_event_date IS NOT NULL
      ...
      AND customer_search_term IS NOT NULL
    GROUP BY
        conversion_event_date, campaign_id, campaign, ad_product_type,
        targeting, match_type, customer_search_term, purchase_currency
),
...
joined AS (
    SELECT
        ...
        t.targeting, t.match_type, t.customer_search_term,
        t.marketplace_name, t.currency_iso_code,
    ...
    LEFT JOIN conversions_clean c
        ...
        AND t.customer_search_term = c.customer_search_term
)
...
FROM joined
$SAN$
WHERE code = 'MC_ZANOM_E003' AND status = 'ACTIVE';
