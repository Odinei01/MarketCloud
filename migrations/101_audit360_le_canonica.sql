-- FIX (auditoria da fase 1, 16/07): refresh_recommendation_hourly_outcomes media
-- o outcome 1h/3h/24h em bronze_ams_hourly — SP-only, nome vazio. A Seladora
-- aplicada pelo auto-apply hoje (janela 1h ja vencida) nao era medida porque a
-- campanha mal aparece nessa fonte. Mesma divida que ja corrigimos no medidor
-- de pin (091) e no silver (083). Passa a ler gold_hourly_signal_unified
-- (canonica, reconciliada, sem negativo), resolvendo campaign_id pelo mapa.
...
BEGIN
    WITH decisions AS (
        ...
        -- FONTE CANONICA (nao bronze_ams_hourly, que e SP-only e tem nome vazio):
        -- gold_hourly_signal_unified reconcilia relatorio+stream e clampa negativos.
        -- Resolve campaign_id pelo mapa de identidade (a canonica so tem nome).
        SELECT
            gi.campaign_id,
        ...
        JOIN marketcloud_gold.gold_campaign_identity gi
          ON gi.campaign_norm = lower(trim(u.campaign_name))
    ), measured AS (
        SELECT
            c.*,
