-- 156_window_stability_and_candidates.sql
-- FUNDACAO das 3 alavancas (bid, placement, preco): score DETERMINISTICO de
-- estabilidade de janela por dia x daypart. "Essa janela converte alto E repete
-- ha N semanas?" Sem isso, placement e preco sao chute. Somente leitura.
--
-- Grao: keyword x daypart x day_bucket (fim de semana / dia util), 6 semanas.
-- Dayparts: madrugada 0-5, manha 6-11, tarde 12-17, noite 18-23.
-- Constantes: TARGET_ROAS=3.0, semana-verde = roas>=3 E >=3 cliques na semana.
--   READY    = >=3 semanas verdes E >=15 cliques totais
--   EMERGING = >=2 semanas verdes E >=8 cliques
--   THIN     = resto (nao autoriza acao de dinheiro)
...
per_week AS (   -- uma linha por janela x semana, marca se a semana foi "verde"
  SELECT keyword_text, campaign_id, campaign_name, daypart, day_bucket, semana,
         clicks, spend, sales,
         (spend > 0 AND clicks >= 3 AND sales/NULLIF(spend,0) >= 3.0) AS week_green
  FROM base
),
agg AS (
  SELECT keyword_text, max(campaign_id) AS campaign_id, max(campaign_name) AS campaign_name,
         daypart, day_bucket,
...
SELECT keyword_text, campaign_id, campaign_name, daypart, day_bucket,
       weeks_active, weeks_green, clicks,
       round(spend::numeric,2) AS spend, round(sales::numeric,2) AS sales, roas,
       CASE
...
-- CANDIDATAS DE PLACEMENT (primeira pagina): grao CAMPANHA x daypart x day_bucket,
-- porque placement (top-of-search) e lever de CAMPANHA na Amazon, nao de keyword.
-- Propoe boost de top-of-search so em janela madura e vencedora. So recomenda.
...
SELECT campaign_id, campaign_name, daypart, day_bucket, clicks,
       ...
       -- boost de top-of-search proposto (deterministico), 0 = nao mexer
       CASE
...
-- CANDIDATAS DE PRICING: janela READY + alta conversao + estavel = hipotese de
-- baixa sensibilidade a preco. NAO propoe preco (isso e teste de elasticidade do
-- robo de pricing) — apenas ALIMENTA a fila de candidatas do robo. So leitura.
