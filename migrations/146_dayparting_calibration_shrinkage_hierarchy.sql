-- 146_dayparting_calibration_shrinkage_hierarchy.sql
...
CREATE OR REPLACE FUNCTION marketcloud_gold.refresh_keyword_hourly_calibration(
    ...
    p_shrink_k         numeric DEFAULT 15    -- constante de shrinkage (cliques p/ 50% de peso proprio)
) RETURNS int
LANGUAGE plpgsql AS $$
DECLARE
...
BEGIN
    ...
    base AS (
        SELECT
            ...
            (SELECT avg_roas FROM globavg) AS ref_roas,
    ...
    SELECT
        ...
        CASE
            ...
            WHEN kc > 0 THEN format('[%s] %sk clk proprios (peso %s) + prior %s: ROAS %s vs ref %s — %s%%->%s%%',
                                    scope_label, round(kc), round(w,2), prior_scope, round(blended_roas,1), round(ref_roas,1), round(published_mult*100), round(tgt*100))
            ELSE format('[%s] sem clique proprio, usa %s: ROAS %s vs ref %s — %s%%->%s%%',
