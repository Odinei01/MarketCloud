-- Migration 023: bronze_amc_hourly_performance → E004 V2 grain
-- V2 removes targeting/match_type from grain (simplified for bid scheduling).
-- ad_group_key surrogate dropped — SQL already emits COALESCE(ad_group, 'NO_AD_GROUP').
-- portfolio_id/name made NOT NULL with NO_PORTFOLIO sentinel.
-- 9 existing rows truncated; V2 backfill will repopulate.

BEGIN;

TRUNCATE marketcloud_bronze.bronze_amc_hourly_performance;

ALTER TABLE marketcloud_bronze.bronze_amc_hourly_performance
    DROP CONSTRAINT bronze_amc_hourly_performance_pkey;

ALTER TABLE marketcloud_bronze.bronze_amc_hourly_performance
    DROP COLUMN targeting,
    DROP COLUMN match_type,
    DROP COLUMN ad_group_key;

ALTER TABLE marketcloud_bronze.bronze_amc_hourly_performance
    ALTER COLUMN ad_group_name SET NOT NULL,
    ALTER COLUMN ad_group_name SET DEFAULT 'NO_AD_GROUP',
    ALTER COLUMN portfolio_id   SET NOT NULL,
    ALTER COLUMN portfolio_id   SET DEFAULT 'NO_PORTFOLIO',
    ALTER COLUMN portfolio_name SET NOT NULL,
    ALTER COLUMN portfolio_name SET DEFAULT 'NO_PORTFOLIO';

ALTER TABLE marketcloud_bronze.bronze_amc_hourly_performance
    ADD PRIMARY KEY (
        tenant_id, amc_instance_id, ads_profile_id,
        data_date, event_hour, campaign_id, ad_product_type, ad_group_name
    );

COMMIT;

-- E004 V2 template (version 2 — version 1 preserved for audit)
INSERT INTO query_templates (
    ...
    'Hourly Performance V2',
    'MC_ZANOM_E004',
    'AMC hourly performance by day/hour/campaign/ad_group for bid scheduling. V2: no targeting/match_type in grain.',
    'BUDGET_WASTE',
    'PERFORMANCE_HOURLY',
    $SQL$
WITH traffic_raw AS (
...
traffic AS (
    SELECT
        ...
        COALESCE(
            NULLIF(TRIM(CAST(ad_group AS VARCHAR)), ''),
            'NO_AD_GROUP'
        ) AS ad_group_name,
        ...
        COALESCE(
            NULLIF(TRIM(CAST(portfolio_id AS VARCHAR)), ''),
            'NO_PORTFOLIO'
        ) AS portfolio_id,

        COALESCE(
            NULLIF(TRIM(CAST(portfolio_name AS VARCHAR)), ''),
            'NO_PORTFOLIO'
        ) AS portfolio_name,
...
$SQL$,
    ...
    '{}', '{"AMAZON_BR"}',
    2, 'ACTIVE'
) ON CONFLICT (code, version) DO UPDATE SET sql_template = EXCLUDED.sql_template, updated_at = NOW();
