-- 183: fontes minimas obrigatorias para decisao Search Intelligence.
-- Objetivo: fechar a matriz Financeiro ASIN + SCP + SQP/Brand Query + Ads Search Terms.
...
adgroup_asin AS (
  SELECT profile_id,
         ...
         MAX(
           CASE
             WHEN ad_group_name ~ 'B0[A-Z0-9]{8}' THEN substring(ad_group_name from '(B0[A-Z0-9]{8})')
             ELSE NULL
           END
         ) AS adgroup_asin
  FROM swarm_src.amazon_ads_targeting_inventory
...
SELECT s.tenant_id,
       ...
       s.ad_product_type,
       s.ad_group_name,
       COALESCE(c.advertised_asin, a.adgroup_asin, i.inferred_asin) AS asin,
       ...
       CASE
         WHEN COALESCE(c.advertised_asin, a.adgroup_asin, i.inferred_asin) IS NOT NULL THEN 'ASIN_RESOLVED'
         ELSE 'ASIN_MISSING'
...
'Ads Search Terms no grao correto para decisao: campanha, grupo, keyword/target, customer_search_term e metricas Ads. ASIN e inferido por report de campanha, nome do ad group ou inventory.';
...
SELECT 'FINANCEIRO_ASIN'::text AS source_key,
       ...
       CASE
         WHEN rows = 0 THEN 'MISSING'
         WHEN rows_with_cost < rows_with_sales THEN 'PARTIAL'
         ELSE 'READY'
       END AS status,
       jsonb_build_object(
         'asins', asins,
...
SELECT 'SEARCH_CATALOG_PERFORMANCE',
       ...
       CASE
         WHEN rows = 0 THEN 'MISSING'
         WHEN impressions <= 0 THEN 'PARTIAL'
         ELSE 'READY'
       END,
       jsonb_build_object(
         'asins', asins,
...
SELECT 'SEARCH_QUERY_PERFORMANCE',
       ...
       CASE
         WHEN rows = 0 THEN 'MISSING'
         WHEN search_query_volume <= 0 THEN 'PARTIAL'
         ELSE 'READY'
       END,
       jsonb_build_object(
         'queries', queries,
...
SELECT 'ADS_SEARCH_TERMS',
       ...
       CASE
         WHEN rows = 0 THEN 'MISSING'
         WHEN search_terms = 0 OR impressions <= 0 THEN 'PARTIAL'
         ELSE 'READY'
       END,
       jsonb_build_object(
         'search_terms', search_terms,
         'resolved_asins', resolved_asins,
         'impressions', impressions,
         'clicks', clicks,
         'spend', spend,
         'orders', orders,
         ...
         'min_date', min_date,
         'max_date', max_date,
         'placement_status', 'MISSING_IN_CURRENT_SOURCE'
       )
FROM ads_terms;

COMMENT ON VIEW marketcloud_gold.gold_search_intelligence_minimum_source_status_v1 IS
'Status das 4 fontes minimas antes de automatizar decisoes: Financeiro ASIN, SCP, SQP/Brand Query e Ads Search Terms.';

CREATE OR REPLACE VIEW marketcloud_gold.gold_search_intelligence_minimum_decision_matrix_v1 AS
WITH finance AS (
  SELECT asin,
...
SELECT f.*,
       ...
       CASE
         WHEN f.gross_sales > 0 AND f.unit_cost > 0 THEN 'READY'
         WHEN f.gross_sales > 0 THEN 'PARTIAL'
         ELSE 'MISSING'
       ...
       CASE
         WHEN f.gross_sales > 0 AND f.unit_cost > 0 AND COALESCE(s.scp_impressions,0) > 0 AND COALESCE(a.ads_search_terms,0) > 0 THEN 'READY_FOR_DECISION'
         WHEN f.gross_sales > 0 AND COALESCE(s.scp_impressions,0) > 0 THEN 'OBSERVE_ONLY'
...
'Matriz por ASIN com financeiro, SCP e Ads Search Terms resolvidos. SQP/Brand Query e uma fonte por query e seu status geral fica em gold_search_intelligence_minimum_source_status_v1.';
