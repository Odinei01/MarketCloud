-- 017_lake_bronze_e003.sql
-- MarketCloud ZANOM — Bronze Search Term Daily
...
INSERT INTO marketcloud_control.extract_definitions (
    ...
    'E003_SEARCH_TERM_DAILY_EXTRACT',
    'Search Term Daily Performance Extract',
    'sponsored_ads',
    ARRAY['sponsored_ads_traffic','amazon_attributed_events_by_conversion_time'],
    'marketcloud_bronze',
    'bronze_amc_search_term_daily',
    'daily',
    35,
    TRUE
)
...
-- ad_group_name pode ser NULL — usamos ad_group_key na PK com default NO_AD_GROUP
CREATE TABLE IF NOT EXISTS marketcloud_bronze.bronze_amc_search_term_daily (
    ...
    PRIMARY KEY (
        ...
        data_date, campaign_id, ad_product_type,
        ad_group_key, targeting, match_type, customer_search_term
    )
);

CREATE INDEX IF NOT EXISTS idx_bronze_search_term_daily_date
    ON marketcloud_bronze.bronze_amc_search_term_daily (tenant_id, data_date);
CREATE INDEX IF NOT EXISTS idx_bronze_search_term_daily_campaign
    ON marketcloud_bronze.bronze_amc_search_term_daily (tenant_id, campaign_id);
CREATE INDEX IF NOT EXISTS idx_bronze_search_term_daily_term
    ON marketcloud_bronze.bronze_amc_search_term_daily (tenant_id, customer_search_term, data_date);
CREATE INDEX IF NOT EXISTS idx_bronze_search_term_daily_targeting
    ON marketcloud_bronze.bronze_amc_search_term_daily (tenant_id, targeting, data_date);
