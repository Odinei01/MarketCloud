-- 148_dayparting_calibration_target_roas.sql
-- FIX CRITICO: referencia era a MEDIA de ROAS (5.8) — cortava horas LUCRATIVAS (ROAS
-- 5.4) so por estarem abaixo da media. Agora referencia = META de ROAS (break-even,
-- default 3.0): hora acima da meta = bid cheio; abaixo = corta proporcional. ROAS 5.4
-- vs meta 3 = sinal 1.8 = 100% (mantem). So corta o que nao paga a meta.
-- CORRECAO: a 144 (gate duro de R$20 no scope) colapsou tudo pro GLOBAL, matando a
-- individualidade por keyword. Errado. O certo e a hierarquia KEYWORD -> CAMPANHA ->
-- GLOBAL com a keyword LIDERANDO, via SHRINKAGE (nao gate duro):
--
--   curva_da_keyword(hora) = w * roas_propria + (1-w) * roas_prior
--   w = cliques_da_keyword_na_hora / (cliques + K)   -- peso do dado proprio
--   prior = campanha (se tiver dado) senao global     -- a hierarquia preenche
--
-- Keyword com dado -> curva propria (abridor de vinho != tag rastreador).
-- Keyword sem dado na hora -> w~0 -> cai no prior (hierarquia). Nunca ruido de 1
-- clique (w pequeno), nunca "global igual pra todas" (quem tem dado diverge).
-- Baseline continua = curva PUBLICADA de cada keyword; recomendacao = delta vs o
-- % dela. So mexe onde ha um prior confiavel (>=2 semanas); senao mantem (madrugada).
...
CREATE OR REPLACE FUNCTION marketcloud_gold.refresh_keyword_hourly_calibration(
    ...
    p_shrink_k         numeric DEFAULT 15,   -- shrinkage (cliques p/ 50% de peso proprio)
    p_target_roas      numeric DEFAULT 3.0   -- meta ROAS (break-even): acima disso = bid cheio
) RETURNS int
LANGUAGE plpgsql AS $$
DECLARE
...
BEGIN
    ...
    base AS (
        SELECT
            ...
            -- PRIOR pela hierarquia: campanha se tiver dado, senao global
        ...
        LEFT JOIN marketcloud_gold.v_published_keyword_hour_mult_v1 pub ON pub.keyword_id=g.keyword_id AND pub.event_hour=g.hr
    ),
    calc AS (
        SELECT *,
            round(kc::numeric / (kc + p_shrink_k), 3) AS w,               -- peso do dado proprio
    ...
    sig AS (
        SELECT *,
            ...
            (prior_weeks >= 2 OR kw_weeks >= 2) AS has_baseline   -- so mexe onde ha historico
        FROM calc
    ),
    decided AS (
        SELECT *,
    ...
    SELECT
        ...
        CASE
            WHEN NOT confident THEN format('sem historico na hora — mantem seu %s%%', round(published_mult*100))
            WHEN kc > 0 THEN format('[%s] %s clk proprios (peso %s) + prior %s: ROAS %s vs meta %s — %s%%->%s%%',
                                    scope_label, round(kc), round(w,2), prior_scope, round(blended_roas,1), round(ref_roas,1), round(published_mult*100), round(tgt*100))
            ELSE format('[%s] sem clique proprio, usa %s: ROAS %s vs meta %s — %s%%->%s%%',
                        scope_label, prior_scope, round(blended_roas,1), round(ref_roas,1), round(published_mult*100), round(tgt*100))
        END
    FROM decided;
