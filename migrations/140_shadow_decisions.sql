-- 140 - Shadow decisions: registra o que o robo FARIA nas campanhas advisor
-- (nao-piloto) e mede o outcome, sem executar. "Modo aprendizado" auditavel.
--
-- (1) constraint aceita execution_status='SHADOW'.
-- (2) refresh_recommendation_hourly_outcomes() passa a medir SHADOW alem de
--     EXECUTED, ancorando em decided_at quando nao ha executed_at.
-- Trava de dinheiro NAO muda: so os 2 pilotos (full_auto) escrevem bid real;
-- os 13 advisor gravam SHADOW e sao medidos, sem tocar a Amazon.

ALTER TABLE marketcloud_recommendations.recommendation_decisions
    DROP CONSTRAINT IF EXISTS chk_execution;
ALTER TABLE marketcloud_recommendations.recommendation_decisions
    ADD CONSTRAINT chk_execution CHECK (execution_status = ANY (ARRAY[
        'NOT_EXECUTED','EXECUTED','SKIPPED','ROLLED_BACK','SHADOW']));
...
BEGIN
    WITH decisions AS (
        SELECT
            d.*,
            COALESCE(d.event_hour, EXTRACT(HOUR FROM COALESCE(d.executed_at, d.decided_at) AT TIME ZONE 'America/Sao_Paulo')::INTEGER) AS measured_hour,
            COALESCE(d.executed_at, d.decided_at) AT TIME ZONE 'America/Sao_Paulo' AS executed_local
        FROM marketcloud_recommendations.recommendation_decisions d
        WHERE d.execution_status IN ('EXECUTED','SHADOW')
          AND d.decision IN ('APPROVED','MODIFIED')
          AND COALESCE(d.executed_at, d.decided_at) IS NOT NULL
