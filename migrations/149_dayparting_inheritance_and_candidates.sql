-- 149_dayparting_inheritance_and_candidates.sql
-- (a) HERANCA: keyword sem schedule proprio nao cai mais no hardcoded do codigo —
--     herda o schedule PUBLICADO pela hierarquia da automacao:
--       keyword (ENTITY) -> grupo (AD_GROUP) -> campanha (CAMPAIGN) -> GLOBAL.
--     So cai no hardcoded se nao houver nem GLOBAL publicado.
-- (b) CANDIDATAS: keyword sem schedule proprio MAS com dado suficiente (>=25 cliques)
--     e sinalizada como candidata a ter schedule proprio.

-- Chaves por scope (confirmado no dado): ENTITY=entity_id/label, AD_GROUP=ad_group_id,
-- CAMPAIGN=campaign_id (entity_id vazio nesses), GLOBAL=todos.
CREATE OR REPLACE VIEW marketcloud_gold.v_effective_published_curve_v1 AS
WITH kw AS (
    SELECT DISTINCT keyword_id, campaign_id, ad_group_id, lower(trim(keyword_text)) AS txt
    FROM marketcloud_bronze.bronze_ams_hourly_target WHERE keyword_id IS NOT NULL
),
prof_hours AS (
    SELECT p.scope,
           p.entity_id, lower(trim(COALESCE(p.entity_label,''))) AS lbl,
           p.campaign_id, p.ad_group_id,
           gs.h::smallint AS event_hour, r.multiplier
    ...
    WHERE p.status='PUBLISHED' AND gs.h >= r.hour_start AND gs.h < r.hour_end
),
matched AS (
    SELECT kw.keyword_id, ph.event_hour, ph.scope, ph.multiplier,
           CASE ph.scope WHEN 'ENTITY' THEN 1 WHEN 'AD_GROUP' THEN 2
                         WHEN 'CAMPAIGN' THEN 3 WHEN 'GLOBAL' THEN 4 ELSE 5 END AS rk
    FROM kw
    JOIN prof_hours ph ON (
             (ph.scope='ENTITY'   AND (ph.entity_id=kw.keyword_id OR (ph.lbl<>'' AND ph.lbl=kw.txt)))
          OR (ph.scope='AD_GROUP' AND kw.ad_group_id IS NOT NULL AND ph.ad_group_id=kw.ad_group_id)
          OR (ph.scope='CAMPAIGN' AND kw.campaign_id IS NOT NULL AND ph.campaign_id=kw.campaign_id)
          OR (ph.scope='GLOBAL')
    )
)
SELECT DISTINCT ON (keyword_id, event_hour)
       keyword_id, event_hour, round(multiplier,2) AS multiplier, scope AS baseline_scope
FROM matched
ORDER BY keyword_id, event_hour, rk;  -- scope mais fino vence

ALTER TABLE marketcloud_gold.gold_keyword_hourly_calibration_v1
    ADD COLUMN IF NOT EXISTS baseline_scope text NOT NULL DEFAULT 'HARDCODED';
...
CREATE OR REPLACE FUNCTION marketcloud_gold.refresh_keyword_hourly_calibration(
    ...
    p_shrink_k         numeric DEFAULT 15,
    p_target_roas      numeric DEFAULT 3.0
) RETURNS int
LANGUAGE plpgsql AS $$
DECLARE
...
BEGIN
    ...
    kwperf AS (SELECT keyword_id, hr, sum(clicks) c, sum(spend) s, sum(sales) v,
    ...
    sig AS (
        SELECT *,
            CASE WHEN ref_roas > 0 THEN round(blended_roas/ref_roas,3) ELSE 1 END AS signal_v
        FROM calc
    ),
    decided AS (
        SELECT *,
            marketcloud_gold._dp_bucket_from_signal(signal_v) AS tgt,
            (prior_weeks >= 2 OR kw_weeks >= 2) AS confident
        FROM sig
    )
    ...
    SELECT
        ...
        round(blended_roas,2), round(ref_roas,2), signal_v,
        tgt,
        ...
        CASE
            ...
            WHEN kc > 0 THEN format('[%s] %s clk proprios (peso %s) + prior %s: ROAS %s vs meta %s — %s%%->%s%% (base: %s)',
            ...
            ELSE format('[%s] sem clique proprio, usa %s: ROAS %s vs meta %s — %s%%->%s%% (base: %s)',
...
-- (b) Candidatas a schedule proprio: sem ENTITY publicado, mas com dado suficiente.
CREATE OR REPLACE VIEW marketcloud_gold.v_dayparting_schedule_candidates_v1 AS
SELECT keyword_id, keyword_text, campaign_id,
       min(baseline_scope) AS herda_de,
       sum(clicks)::int AS clicks_total,
       count(*) FILTER (WHERE gate='OK' AND action<>'HOLD') AS horas_com_rec
FROM marketcloud_gold.gold_keyword_hourly_calibration_latest_v1
WHERE baseline_scope <> 'ENTITY'
GROUP BY 1,2,3
