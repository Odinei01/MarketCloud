-- Migration 022: bronze_amc_new_to_brand_halo_daily (E008)
-- Grain: conversion_date / campaign_id / ad_product_type
--        / targeting / match_type / customer_search_term / tracked_asin
--
-- Source: amazon_attributed_events_by_conversion_time (conversion-only, no spend).
-- Cross with E001/E002/E003 in Silver for CPA and ROAS by NTB/halo segment.

CREATE TABLE IF NOT EXISTS marketcloud_bronze.bronze_amc_new_to_brand_halo_daily (
    -- Identity
    ...
    brand_halo_sales_share    NUMERIC(10,8) NOT NULL DEFAULT 0,
    ...
    click_purchase_share      NUMERIC(10,8) NOT NULL DEFAULT 0,
    click_sales_share         NUMERIC(10,8) NOT NULL DEFAULT 0,

    loaded_at       TIMESTAMPTZ NOT NULL DEFAULT NOW(),

    PRIMARY KEY (
        tenant_id, amc_instance_id, ads_profile_id,
        conversion_date, campaign_id, ad_product_type,
        targeting, match_type, customer_search_term, tracked_asin
    )
);

CREATE INDEX IF NOT EXISTS idx_bronze_ntb_date
    ON marketcloud_bronze.bronze_amc_new_to_brand_halo_daily (tenant_id, amc_instance_id, conversion_date);

CREATE INDEX IF NOT EXISTS idx_bronze_ntb_campaign
    ON marketcloud_bronze.bronze_amc_new_to_brand_halo_daily (tenant_id, amc_instance_id, campaign_id);

CREATE INDEX IF NOT EXISTS idx_bronze_ntb_asin
    ON marketcloud_bronze.bronze_amc_new_to_brand_halo_daily (tenant_id, amc_instance_id, tracked_asin);

-- E008 extract definition
INSERT INTO marketcloud_control.extract_definitions (
    ...
    'E008_NEW_TO_BRAND_HALO_EXTRACT',
    'New to Brand and Brand Halo Daily Extract',
    'AMC_SPONSORED_ADS',
    ARRAY['amazon_attributed_events_by_conversion_time'],
    'marketcloud_bronze',
    'bronze_amc_new_to_brand_halo_daily',
    'DAILY',
    35,
    true
