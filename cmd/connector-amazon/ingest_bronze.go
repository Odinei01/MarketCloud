...
// POST /internal/amc/ingest/e001/{execution_id}
// Downloads the E001 CSV from S3 and upserts rows into
// marketcloud_bronze.bronze_amc_campaign_daily.
// Body: { "tenant_id", "amc_instance_id", "ads_profile_id" }
func (s *connectorServer) ingestE001(w http.ResponseWriter, r *http.Request) {
	...
	// Look up the S3 path and workflow_run_id (we use the query_run id as workflow_run reference)
	var s3Path string
	...
	for {
		...
		if err != nil {
			if _, ok := err.(*csv.ParseError); ok {
				log.Printf("ingest e001: csv parse error (linha ignorada): %v", err)
				continue
			}
			log.Printf("ingest e001: csv read abortado (I/O), parando: %v", err)
			break
		}
		...
		// AMC date formats: "2026-07-01" or "2026-07-01T00:00:00Z"
		dataDate := rawDate
		if len(dataDate) > 10 {
		...
		_, err = s.db.Exec(r.Context(), `
			INSERT INTO marketcloud_bronze.bronze_amc_campaign_daily (
				tenant_id, amc_instance_id, ads_profile_id, workflow_run_id,
				data_date, campaign_id, campaign_name, ad_product_type,
				marketplace_id, marketplace_name, currency_iso_code, purchase_currency,
			...
			ON CONFLICT (tenant_id, amc_instance_id, ads_profile_id, data_date, campaign_id, ad_product_type)
			DO UPDATE SET
		...
		`,
			...
			col(row, "marketplace_id"), col(row, "marketplace_name"),
		...
		if err != nil {
			log.Printf("ingest e001: upsert error row campaign=%s date=%s: %v", campaignID, dataDate, err)
			skipped++
			continue
		}
		inserted++
	...
	log.Printf("ingest e001: execution=%s inserted=%d skipped=%d", executionID, inserted, skipped)
...
func toInt64(s string) int64 {
	...
	v, _ := strconv.ParseFloat(s, 64) // AMC may emit "127" or "1.27e2"
	return int64(v)
}
