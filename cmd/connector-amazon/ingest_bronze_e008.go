...
// POST /internal/amc/ingest/e008/{execution_id}
// Downloads E008 CSV from S3 and upserts into marketcloud_bronze.bronze_amc_new_to_brand_halo_daily.
// Body: { "tenant_id", "amc_instance_id", "ads_profile_id" }
//
// E008: conversion-only NTB/brand-halo metrics. No spend, no ad_group.
// Cross with E001/E002/E003 in Silver for CPA and ROAS by segment.
func (s *connectorServer) ingestE008(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		if err != nil {
			if _, ok := err.(*csv.ParseError); ok {
				log.Printf("ingest e008: csv parse error (linha ignorada): %v", err)
				continue
			}
			log.Printf("ingest e008: csv read abortado (I/O), parando: %v", err)
			break
		}
		...
		trackedAsin := col(row, "tracked_asin")

		if campaignID == "" || campaignName == "" || adProductType == "" || trackedAsin == "" {
			skipped++
			continue
		}

		rawDate := col(row, "conversion_date")
		if rawDate == "" {
			skipped++
			continue
		}
		conversionDate := rawDate
		if len(conversionDate) > 10 {
			conversionDate = conversionDate[:10]
		}
		...
		_, err = s.db.Exec(r.Context(), `
			INSERT INTO marketcloud_bronze.bronze_amc_new_to_brand_halo_daily (
				tenant_id, amc_instance_id, ads_profile_id, workflow_run_id,
				conversion_date, campaign_id, campaign_name, ad_product_type,
				...
				brand_halo_order_share, brand_halo_sales_share,
				new_to_brand_order_share, new_to_brand_sales_share,
				average_order_value,
				click_purchase_share, click_sales_share,
				loaded_at
			) VALUES (
				$1,$2,$3,$4,
				$5,$6,$7,$8,
			...
			ON CONFLICT (tenant_id, amc_instance_id, ads_profile_id, conversion_date, campaign_id, ad_product_type, targeting, match_type, customer_search_term, tracked_asin)
			DO UPDATE SET
				...
				brand_halo_order_share    = EXCLUDED.brand_halo_order_share,
				brand_halo_sales_share    = EXCLUDED.brand_halo_sales_share,
				new_to_brand_order_share  = EXCLUDED.new_to_brand_order_share,
				new_to_brand_sales_share  = EXCLUDED.new_to_brand_sales_share,
				average_order_value       = EXCLUDED.average_order_value,
				click_purchase_share      = EXCLUDED.click_purchase_share,
				click_sales_share         = EXCLUDED.click_sales_share,
				workflow_run_id           = EXCLUDED.workflow_run_id,
				loaded_at                 = NOW()
		`,
			req.TenantID, req.AMCInstanceID, req.AdsProfileID, workflowRunID,
			conversionDate, campaignID, campaignName, adProductType,
			targeting, matchType, customerSearchTerm, trackedAsin,
			...
			toFloat(col(row, "brand_halo_order_share")), toFloat(col(row, "brand_halo_sales_share")),
			toFloat(col(row, "new_to_brand_order_share")), toFloat(col(row, "new_to_brand_sales_share")),
			toFloat(col(row, "average_order_value")),
			toFloat(col(row, "click_purchase_share")), toFloat(col(row, "click_sales_share")),
		)
		if err != nil {
			log.Printf("ingest e008: upsert error asin=%s campaign=%s date=%s: %v", trackedAsin, campaignID, conversionDate, err)
			skipped++
			continue
		}
		inserted++
	...
	log.Printf("ingest e008: execution=%s inserted=%d skipped=%d", executionID, inserted, skipped)
