...
func (s *connectorServer) ingestE004(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		if err != nil {
			if _, ok := err.(*csv.ParseError); ok {
				log.Printf("ingest e004: csv parse error (linha ignorada): %v", err)
				continue
			}
			log.Printf("ingest e004: csv read abortado (I/O), parando: %v", err)
			break
		}
	...
	log.Printf("ingest e004: execution=%s inserted=%d skipped=%d", executionID, inserted, skipped)
