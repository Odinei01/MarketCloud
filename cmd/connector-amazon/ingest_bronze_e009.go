...
// POST /internal/amc/ingest/e009/{execution_id}
// Downloads E009 CSV from S3 and upserts into marketcloud_bronze.bronze_amc_conversions_unified_daily.
// Body: { "tenant_id", "amc_instance_id", "ads_profile_id" }
//
// E009: UNION ALL of CONVERSION_TIME + TRAFFIC_TIME attribution modes.
// attribution_mode is part of PK — same campaign/asin/date row appears twice (one per mode).
// tracked_asin uses NO_ASIN sentinel for add_to_cart/dpv events without a purchased ASIN.
func (s *connectorServer) ingestE009(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		if attributionMode == "" || campaignID == "" || campaignName == "" || adProductType == "" {
			skipped++
			continue
		}

		rawDate := col(row, "attribution_date")
		if rawDate == "" {
			skipped++
			continue
		}
		attributionDate := rawDate
		if len(attributionDate) > 10 {
			attributionDate = attributionDate[:10]
		}
		...
		_, err = s.db.Exec(r.Context(), `
			INSERT INTO marketcloud_bronze.bronze_amc_conversions_unified_daily (
				tenant_id, amc_instance_id, ads_profile_id, workflow_run_id,
				attribution_mode, attribution_date,
				...
				brand_halo_order_share, new_to_brand_order_share, new_to_brand_sales_share,
				click_attribution_share, average_order_value,
				loaded_at
			) VALUES (
				$1,$2,$3,$4,
				$5,$6,
			...
			ON CONFLICT (tenant_id, amc_instance_id, ads_profile_id, attribution_mode, attribution_date, campaign_id, ad_product_type, targeting, match_type, customer_search_term, tracked_asin)
			DO UPDATE SET
		...
		`,
			...
			attributionMode, attributionDate,
			...
			toFloat(col(row, "brand_halo_order_share")),
			toFloat(col(row, "new_to_brand_order_share")), toFloat(col(row, "new_to_brand_sales_share")),
			toFloat(col(row, "click_attribution_share")), toFloat(col(row, "average_order_value")),
		)
		if err != nil {
			log.Printf("ingest e009: upsert error mode=%s asin=%s campaign=%s date=%s: %v",
				attributionMode, trackedAsin, campaignID, attributionDate, err)
			skipped++
			continue
		}
		inserted++
	...
	log.Printf("ingest e009: execution=%s inserted=%d skipped=%d", executionID, inserted, skipped)
