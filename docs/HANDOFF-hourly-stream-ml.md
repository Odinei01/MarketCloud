# Handoff â€” Camada HorÃ¡ria Real + ML + Amazon Marketing Stream

> Documento de passagem de bastÃ£o. LÃª inteiro antes de mexer. Estado congelado
> em **2026-07-08**. Dois repositÃ³rios envolvidos:
> - `marketcloud/` (ZMC â€” lake AMC, Gold, ML, cockpit React, API Go, Postgres `marketcloud_db`)
> - `mercado-data-app/` (SWARM/RobÃ´ ZANOM â€” Postgres `pricing_db`/`pricing_intelligence`)
...
Cloud), que no grÃ£o horÃ¡rio **suprime ~74% das conversÃµes** por privacidade
(agrega vÃ¡rios anunciantes). O modelo via `has_order = 1/720` e sÃ³ sabia
replicar a regra do Gold â€” inÃºtil.

**Virada:** usar o **dado real da prÃ³pria conta** (nÃ£o suprimido). Provamos que
o relatÃ³rio horÃ¡rio da conta traz 224 pedidos vs 49 do AMC no mesmo perÃ­odo. Com
isso o ML ganha um alvo real (ROAS/conversÃ£o por horaÃ—campanha) e passa a
aprender de verdade.

**Como manter esse dado fluindo automÃ¡tico:** a API de *reporting* (pull) da
Amazon **NÃƒO** entrega grÃ£o horÃ¡rio (testado: HTTP 400 `timeUnit is not
supported`). O caminho Ã© **Amazon Marketing Stream (push)** â€” a Amazon empurra,
hora-a-hora, os datasets `sp-traffic` (impressÃµes/cliques/gasto) e
`sp-conversion` (pedidos/vendas por janela de atribuiÃ§Ã£o) para filas **SQS** na
nossa conta AWS. Um consumidor lÃª e grava no lake.

**PrincÃ­pio inegociÃ¡vel:** tudo Ã© **advisor**. Nada executa lance/orÃ§amento/
negativa na Amazon. O cockpit sugere; humano/RobÃ´ decide.
...
```
Amazon Ads â”€â”€push h/hâ”€â”€â–¶ SQS (conta 508859666731, us-east-1)
                              â”‚
                    consumidor Go (marketcloud api)
                              â”‚  upsert idempotente (last-write-wins)
                              â–¼
      marketcloud_bronze.bronze_ams_hourly  (chaveado por campaign_id)
                              â”‚  [OK: refresh_ams_to_hourly resolve campaign_idâ†’name]
                              â–¼
      marketcloud_bronze.bronze_amazon_ads_hourly  (chaveado por campaign_name)
                              â”‚
             â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”´â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
             â–¼                                   â–¼
   Gold horÃ¡rio (views 051/053)        ML V2 (workers/ml-worker)
   gold_hourly_recommendations_v1      HourlyConversionRealV2 (AUC 0.956)
   BID_UP / CUT_HOUR / BID_DOWN        HourlyExpectedRoasRealV2
             â”‚                                   â”‚
             â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–º cockpit React â—„â”€â”€â”€â”€â”€â”€â”€â”˜
                 pÃ¡gina "HorÃ¡rios (real)" (Gold Ã— ML)
```

---

## 3. O que ESTÃ PRONTO (verificado nesta sessÃ£o)

### 3.1 Dado horÃ¡rio real (bridge CSV, manual â€” funciona hoje)
- `migrations/050_lake_amazon_ads_hourly.sql` â†’ `bronze_amazon_ads_hourly`.
- Carregado 31/05â†’08/07: **8.148 linhas, 224 pedidos, R$8.672**. (IngestÃ£o dos
  CSVs do console via staging + `DISTINCT ON` por overlap de datas.)

### 3.2 Gold horÃ¡rio sobre dado real
- `migrations/051_gold_hourly_real_v1.sql` â†’ `gold_hourly_perf_v1` +
  `gold_hourly_recommendations_v1`. Cruza campanhaÃ—hora real Ã— agenda de
  multiplicadores do RobÃ´ (`bronze_swarm_bid_schedule`, join por NOME). AÃ§Ãµes
  BID_UP/CUT_HOUR/BID_DOWN/KEEP_STRONG, confianÃ§a por volume, `label_caveat`.
- `migrations/053_gold_hourly_join_ml.sql` â†’ adiciona colunas ML
  (`ml_conversion_probability`, `ml_expected_roas`, `ml_agrees`).
- Exemplos reais validados: `Localizador 21h` ROAS 8.93 estrangulada a 0.5Ã— â†’
  BID_UP; `AutomÃ¡tica 22h` R$120 ROAS 1.48 â†’ BID_DOWN.
...
- `migrations/052_ml_hourly_real_predictions_v2.sql` â†’ `hourly_ml_predictions_v2`.
- Resultado (out-of-fold, sem vazamento): **ConversÃ£o AUC 0.956** vs baseline
  0.700; **ROAS MAE 1.305** vs 2.057. CalibraÃ§Ã£o: horas previstas "boas" (50) â†’
  **98% converteram** (ROAS real 13.44); nÃ£o-boas â†’ 10%.
- Agendado no `modeling-worker` a cada 60 min; tambÃ©m validado manualmente via
  `docker exec`.

### 3.4 API + cockpit
- `GET /api/v1/gold/hourly-real` em `internal/query/gold_v2.go` (rota em
  `cmd/api/main.go`). Filtros: action, confidence, include_keep.
- Frontend: `frontend/src/pages/HorariosReais.jsx`, menu "â—· HorÃ¡rios (real)"
  em `App.jsx`. Mostra Gold Ã— ML com veredito de concordÃ¢ncia.

### 3.5 Amazon Marketing Stream (a parte nova â€” quase completa)
**Lado descoberta (SWARM):**
- `mercado-data-app/internal/services/amazon_ads_hourly.go`:
  - Report horÃ¡rio via pull â†’ provado que **nÃ£o existe** (400).
  - `GET /api/amazon/ads/stream/eligibility` â†’ **AMS LIBERADO** (200, profile BR
    3084626225435227, realm NA, zero subscriptions).

**Lado AWS (Fase 1 â€” FEITO pelo DevOps):**
...
- Conta **508859666731 / Zanom Digital**, `us-east-1`. Filas criadas:
  ...
  - + DLQs.
- IAM user consumidor: `marketcloud-ams-consumer` (sÃ³ leitura das filas).

**Lado app (Fase 2+3 â€” CÃ“DIGO PRONTO, ligado):**
- `internal/stream/subscriptions.go` â€” cria/lista/remove subscriptions via Ads
  API. Reusa OAuth (`amazon_oauth_connections`, store Zanom
  `f1a59d8d-2966-45c1-83be-8e20c87ea1e0`, refresh LWA automÃ¡tico).
- `internal/stream/consumer.go` â€” long-poll SQS (aws-sdk-go-v2), desembrulha
  envelope SNS (auto-confirma `SubscriptionConfirmation`), **upsert idempotente
  last-write-wins** em `bronze_ams_hourly`.
- `migrations/054_bronze_ams_hourly.sql` â†’ landing chaveado por campaign_id,
...
- Config nova (`internal/config/config.go`): `STREAM_CONSUMER_ENABLED`,
  ...
  separada da AWS_* do AMC pra nÃ£o colidir), `AMAZON_ADS_PROFILE_ID`.
...
**Atualizado em 08/07, noite:** o blocker de destino SQS foi resolvido no §10.
...
O campo (`destinationArn`) Ã© reconhecido (a Amazon ecoa o ARN), mas ela **recusa
a fila** â€” Ã© o prÃ©-check de acesso cross-account. A `clientRequestToken` jÃ¡ foi
corrigida (usa UUID â‰¤ 36 chars); esse erro sumiu, sobra sÃ³ o destino.

**Confirmado depois:** a *policy* da fila (antes `arn:aws:sns:us-east-1:*:*`) nÃ£o
concede Ã  conta/serviÃ§o especÃ­fico do AMS o que ele exige pra validar entrega.
...
### 5.1 ðŸŸ¢ Subscription AMS
**FEITO no §10.1.** Nao recriar sem antes listar as subscriptions existentes.
Usar `GET /api/v1/stream/subscriptions/` para confirmar status.

### 5.2 ðŸŸ  Confirmar dados chegando
- ApÃ³s a subscription, a Amazon manda `SubscriptionConfirmation` â†’ o consumidor
  auto-confirma (ver log `[ams-stream] SNS SubscriptionConfirmation confirmada`).
- Em ~1h comeÃ§am os registros. Conferir:
...
### 5.3 ðŸŸ¢ ReconciliaÃ§Ã£o bronze_ams_hourly â†’ bronze_amazon_ads_hourly
...
# migrations: aplicar 050â†’054 (docker exec -i marketcloud_db psql -U mcadmin -d marketcloud -f - < migrations/0XX.sql)
...
## 7. SeguranÃ§a / constraints (NÃƒO violar)

- **Nunca commitar:** `Zanom_MktCloud_Amz.csv`, credenciais AWS, `.env` (gitignored),
  `*.csv`/`*.exe`/`*.pkl`. O secret do consumidor vive sÃ³ no `.env` local.
- **ZANOM safety:** negativas nunca podem conter termos com 'zanom' (Gold G005
  forÃ§a WATCH/SAFETY_BLOCKED).
- **Advisor-only:** nenhuma mutaÃ§Ã£o na Amazon (bid/budget/negativa/API). O Gold
  sugere; o cockpit registra decisÃ£o humana; execuÃ§Ã£o Ã© fora daqui.
- **fdw** `marketcloud_db â†’ pricing_db` (server `swarm_pg`, schema `swarm_src`):
...
## 7.1 Progresso pÃ³s-handoff (08/07, noite) — historico superado pelo §10

AvanÃ§os self-contained (sem depender do blocker):
- **Corpo da subscription CONFIRMADO correto** â€” exemplo oficial da Amazon usa
  exatamente `{clientRequestToken, dataSetId, notes, destinationArn}`, igual ao
  `buildSubscriptionBody`. O `destinationArn` com ARN de SQS puro tambÃ©m Ã© o
  formato certo. Ou seja, o 400 **nÃ£o Ã© o corpo**.
- **Media type corrigido** â†’ `...StreamSubscriptionResource.v1.0+json` (era `v1`).
- **Â§5.3 FEITO** â€” `migrations/055_reconcile_ams_to_hourly.sql`: view
  `v_ams_hourly_resolved` (resolve campaign_idâ†’name via mÃ©tricas/agenda do SWARM)
  + funÃ§Ã£o `marketcloud_bronze.refresh_ams_to_hourly()` que faz upsert no
  `bronze_amazon_ads_hourly` (o que o Gold lÃª). Testada (0 linhas hoje, roda). O
  prÃ³ximo dev sÃ³ precisa **chamar essa funÃ§Ã£o** depois que o Stream popular o
  `bronze_ams_hourly` (idealmente no fim do `pollLoop` do consumidor, ou num
  worker periÃ³dico).
- **Historico do blocker Â§5.1**: NÃƒO era corpo nem media type. Era a **policy da fila** â€”
  o principal/account-id exato do AMS nÃ£o Ã© pÃºblico (nÃ£o estÃ¡ no blog/CDK
  ...
  no cÃ³digo do CDK `amzn/amazon-marketing-stream-examples` (construct de SQS).
  Resolvido no §10 com SourceArns oficiais por dataset e `ReviewerRole`.

Ainda pendente: validar fuso e payload real quando a primeira entrega chegar. ML
ja esta agendado.

## 7.2 Fase 4 â€” datasets AMS extras (depois do pipeline de pÃ©)

O AMS entrega ~45 datasets (SP/SB/SD Ã— traffic/conversion/budget/change/
diagnostics; lista autoritativa: https://advertising.amazon.com/API/docs/en-us/amazon-marketing-stream/data-guide).
Hoje sÃ³ usamos `sp-traffic` + `sp-conversion`. Quando o pipeline estiver
fluindo, vale assinar mais dois â€” mesmo encanamento (nova subscription + nova
fila SQS + roteamento no consumidor):

- **`budget-usage`** (consumo de orÃ§amento quase em tempo real) â†’ cruzar com a
  Q009 "campanha boa sem orÃ§amento" e agir antes da campanha estourar no meio do
  dia. Precisa de tabela landing prÃ³pria + regra no Gold.
- **Change notifications** (mudanÃ§as em campanha/ad group/target â€” event-driven)
  â†’ gravar no lake TODA alteraÃ§Ã£o que o RobÃ´/humano faz. Isso **fecha o loop
  causal** que perseguimos: cockpit sugere â†’ alguÃ©m aplica â†’ o change event
  registra o "quando/o quÃª" â†’ o `sp-traffic`/`sp-conversion` mede o "depois" â†’
  vira label causal (nÃ£o mais observacional) pro ML. Ã‰ o elo que transforma o ML
  de "advisor observacional" em "aprende efeito real da aÃ§Ã£o".

ImplementaÃ§Ã£o: subir 1 fila SQS por dataset novo no Terraform (`datasets` jÃ¡ Ã©
...
## 7.3 Fase 6 — recomendações no grão de KEYWORD (não só campanha)

Pedido do dono: descer o grão das recs horárias de campanha×hora para
**keyword×hora** (ex.: "tag rastreador android → lance efetivo R$1,00 às 14h"),
não só "campanha Localizador reduz multiplicador às 15h".

**O que trava NÃO é o robô — é o dado.** A execução por keyword já existe (o
Robô ZANOM faz lance por palavra via `amazon_ads_bid_decisions`). O gargalo é a
fonte no grão horário:
- O relatório do console (CSV ingerido) é **só campanha×hora** — sem keyword.
- O **AMS** (`sp-traffic`/`sp-conversion`) *pode* trazer `keywordId`/`targeting`/
  ...
  captura só o nível de campanha e descarta keyword). Verificar na 1ª entrega.

**Ressalva estatística (séria):** keyword×hora é **muito esparso** — a maioria das
células terá 0-1 clique. Lance confiável só nos **top keywords com volume**; o
long tail vira ruído nesse grão.

**Mecanismo correto (como a Amazon faz):** o ajuste por hora é um **multiplicador**
sobre o lance-base da keyword, não um lance absoluto por hora. Logo o modelo
honesto **combina dois grãos**:
- **lance-base por keyword** ← dado **diário** (denso, robusto; o Robô já calcula).
- **forma por hora** ← dado **horário** (grão mais denso: campanha/ad group).
- `lance efetivo às H = lance_base_keyword × multiplicador_da_hora`. Mostra no
  grão da keyword, mas a alavanca é base-bid (diário) ou multiplicador (horário).

**Plano:**
1. Confirmar se o AMS entrega `keywordId` por hora (1º payload real — depende da
   Fase de validação §10.6). Se NÃO vier, keyword×hora não existe na fonte e fica
   só base-bid diário × multiplicador de campanha.
2. Se vier: capturar o grão de keyword num landing próprio (`bronze_ams_hourly_target`,
   chave data/hora/campaign_id/keyword_id ou target_id).
3. Gold keyword×hora **com trava de confiança forte** (ex.: só recomenda se
   ≥N cliques/hora no keyword); abaixo disso, herda o multiplicador da campanha.
   Sem inventar lance sobre ruído — logar o que foi suprimido por baixa amostra.
4. Recomendação = ajuste no lance-base (esteira diária do Robô) OU no multiplicador
   da hora; execução pela esteira de keyword-bid que o Robô já tem (dry_run primeiro,
   ver §7.4 se existir; senão advisor no cockpit).

Status 08/07 noite: a Fase 6 foi implementada no modo seguro/advisor descrito
acima. Nao inventamos dado keyword×hora: o cockpit agora mostra
`lance efetivo = lance_base_keyword × multiplicador_horario_da_campanha`, e a
...
Dado horÃ¡rio real prova o ML (AUC 0.956) e alimenta o Gold/cockpit; o pipeline
automÃ¡tico (Amazon Marketing Stream â†’ SQS â†’ consumidor â†’ lake) estÃ¡ **codado,
...
Proximos passos imediatos (historico; ver §10 para estado atual):
...
- Consultei o repositório oficial `amzn/amazon-marketing-stream-examples` e confirmei em `stream_infrastructure_config.yml` os SourceArns oficiais do realm NA:
  - `sp-traffic`: `arn:aws:sns:us-east-1:906013806264:*`
  - `sp-conversion`: `arn:aws:sns:us-east-1:802324068763:*`
- Também repliquei o comportamento do CDK oficial: conceder `sqs:GetQueueAttributes` para `arn:aws:iam::926844853897:role/ReviewerRole`.
- Promovi o Terraform real para `C:\dev\estudo-cloud-native\marketcloud\infra\ams-stream` e copiei o state local para lá. A partir de agora, este é o diretório operacional correto para `terraform plan/apply`.
...
- Execucao imediata terminou com sucesso:
  - `586 células campanha×hora | 90 com pedido | 496 sem`
  - `Conversão: AUC=0.956 baseline=0.700 beats=True`
  - `ROAS: MAE=1.305 r2=0.152 baseline_MAE=2.057 beats=True`
  - `586 predições gravadas em hourly_ml_predictions_v2`
...
## 11. Uso real + loop causal + fallback (08/07, noite — dono operando)

O dono começou a **aplicar as recomendações** da tela "Horários — Dado Real"
(subir/amaciar multiplicadores no Robô). Isso é o loop pretendido funcionando:
ele age → o sync horário do SWARM atualiza `bronze_swarm_bid_schedule` → o cockpit
esconde o que já foi feito (por isso 29 → 17 oportunidades numa atualização).

### 11.1 Estado da tela: "meio ao vivo"
Duas cadências distintas alimentam a tela hoje:
- **Agenda do Robô** (mult. atual, "já feito"): **VIVO**, sync de hora em hora.
  Foi o que mudou 29→17 — o Robô já subiu algumas horas estranguladas.
- **ML** (concorda/prob/ROAS esperado): re-treina de hora em hora, mas sobre a
  performance estática.
- **Performance** (ROAS/gasto/pedidos por hora): **CONGELADA no CSV** (31/05→08/07)
  até o AMS fluir. Por isso a janela ainda diz `…→2026-07-08`.

Consequência honesta: o dono está agindo **em malha aberta** — ao subir um lance,
o cockpit ainda NÃO mede se o ROAS se manteve (o "depois" só vem com o AMS).

### 11.2 Baseline causal (a fazer quando aplicar mudanças)
Para transformar as mudanças de agora nos primeiros pontos de dado CAUSAL, capturar
um snapshot "antes" (multiplicador antigo + ROAS naquele multiplicador + ação
recomendada + timestamp) — sugerido: tabela `marketcloud_gold.hourly_action_baseline`.
Quando o AMS entregar o "depois", medir o delta por hora×campanha (bid subiu → ROAS
segurou ou caiu?). É o elo que tira o ML do observacional. NÃO foi criada ainda —
oferecida ao dono; fazer no próximo passe se ele topar (o baseline de multiplicador
é perecível conforme o Robô sincroniza).

### 11.3 Decisão operacional: fallback CSV
Se o AMS **não entregar até virar o dia**, o dono vai **puxar o CSV do console
manualmente** (mesmo processo da §3.1: staging + `DISTINCT ON`, upsert em
`bronze_amazon_ads_hourly`) para manter a performance fresca enquanto o Stream não
assume. O CSV continua sendo a ponte de contingência até o AMS provar entrega.

### 11.4 Status AMS neste momento
`bronze_ams_hourly` = 0; filas SQS `0/0`; subscriptions ACTIVE desde 22:37 UTC.
Monitor `bcx76ak04` rodando (checa a cada 5 min, ~4h). Janela normal de 1ª entrega
até ~01:00–01:30 UTC; se passar zerado, investigar o `SourceArn` da policy (se o
AMS publica de topic/conta diferente de 906013806264/802324068763 para este profile).

## 12. Fase 6 implementada — keyword×hora advisor (08/07, noite)

Objetivo atacado: descer a recomendacao horaria para o grao que o dono pediu
(`keyword × hora`), sem fingir que ja existe payload AMS por keyword. O resultado
atual e advisor-only:

`lance efetivo na hora = base_bid_da_keyword × multiplicador_horario_da_campanha`
...
- Resultado atual: `69` recomendacoes keyword×hora.
...
Mudanca:
- Continua fazendo upsert campanha×hora em `bronze_ams_hourly`.
...
### 12.8 Ajuste visual da tela keyword×hora
...
### 13.7 REGRA de escalonamento refinada (não recriar cedo)

**NÃO recriar as subscriptions antes de ~03:30–04:00 UTC.** Recriar **reseta o
relógio de entrega** do AMS (ele recomeça a contar horas do zero → reinicia o
warm-up), então recriar durante um warm-up só atrasa. Como as causas de falha
PERMANENTE já foram descartadas (KMS §13.3, SourceArn §13.4), o cenário dominante
é latência da 1ª entrega (normal 1–3h).

- Aguardar até **~03:30–04:00 UTC** (2–3 horas elegíveis acumuladas).
- Só então, se continuar zerado: **recriar** (DELETE+POST) **+ abrir caso no
  Amazon Ads support** ("subscription ACTIVE, sem entrega SQS"). Com 3h elegíveis
  zeradas deixa de ser warm-up e vira problema estrutural.
- O CloudWatch (§13.2, precisa root) é o que confirma objetivamente: `Sent>0` em
  qualquer hora = entregou; `0` em 3h elegíveis = não entregou.
- Rede de segurança: fallback CSV (§11.3) mantém a performance fresca enquanto isso.

## 14. Registro paralelo — unificação de custo Amazon/estoque (mercado-data-app)
...
Decisão implementada: nos cálculos de CMV/margem/listas Amazon, o custo efetivo
...
## 15. RUNBOOK de recuperação AMS — fila v2 + re-subscribe (receita da Amazon)

**Contexto (09/07 ~02:19 UTC):** subscriptions AMS `ACTIVE`, mas ~3h42 e 3 horas
elegíveis (23-00, 00-01, 01-02 UTC) com ZERO entrega; filas `0/0`, consumidor
nunca recebeu nada — nem a mensagem de `SubscriptionConfirmation`. KMS descartado
(§13.3) e SourceArn confirmado (§13.4). Padrão bate com a orientação oficial da
Amazon: *"status pode dizer ACTIVE, mas se você nunca recebeu a confirmação no SQS,
a policy/fila pode estar errada pra região/dataset — crie uma fila NOVA, aplique a
policy correta e re-subscribe com o novo ARN"* (não dá pra reconfirmar a mesma fila).

### 15.1 GATILHO (quando executar)
Executar quando **qualquer** um for verdade:
- CloudWatch (via root, §13.2) mostrar `NumberOfMessagesSent = 0` ao longo das
  horas elegíveis (prova objetiva de não-entrega); OU
- Continuar `0` em tudo após **~03:30–04:00 UTC**.
Se o dado cair antes disso, era latência — **não executar** (v2 reseta o relógio).

### 15.2 Passos (a AWS é do dono/DevOps; eu preparo, eles aplicam)
...
1) **Filas novas via Terraform** (`marketcloud/infra/ams-stream`). Bumpar o prefixo
   ...
   ```
   `terraform apply` → cria 6 recursos novos (`zanom-ams-v2-sp-traffic-ingress`,
   `zanom-ams-v2-sp-conversion-ingress`, +DLQs, +policies com o MESMO SourceArn
   correto) e destrói os 6 antigos. Pegar os novos ARNs/URLs:
   ...
   inline para os ARNs `-v2` — precisa de root/admin.)

2) **App aponta pras filas novas** — no `.env` do marketcloud:
   ```
...
O que faltou desta vez: **a mensagem de confirmação chegando na fila**. Após o
re-subscribe, procurar no log:
```
[ams-stream] SNS SubscriptionConfirmation confirmada (sp-traffic)
[ams-stream] SNS SubscriptionConfirmation confirmada (sp-conversion)
```
Se aparecer → o SNS→SQS foi confirmado de verdade → dado começa a fluir na próxima
hora cheia. Conferir `SELECT COUNT(*) FROM marketcloud_bronze.bronze_ams_hourly;`.
Se NÃO aparecer nenhuma confirmação de novo → o problema é upstream no AMS/profile
→ abrir caso no Amazon Ads support com os subscriptionIds.

### 15.4 Notas
- **Não** recriar a fila v2 mais de uma vez sem antes ter a prova (CloudWatch) —
  cada recriação reseta o warm-up.
- Fallback CSV (§11.3) segue mantendo a performance fresca.
- Depois que o v2 funcionar e estabilizar, remover os recursos antigos já saiu no
  `apply` (o Terraform destrói os `zanom-ams-*` sem sufixo).

### 15.5 Execucao do runbook v2 (09/07/2026 02:30–02:33 UTC)
...
### 15.6b Registro intermediario SUPERADO — orfas antes do cleanup (09/07 ~02:35 UTC)
SUPERADO por §15.6: as subscriptions v1 orfas foram arquivadas com sucesso as
...
- **v2 destravou:** subscriptions v2 ACTIVE + **`SubscriptionConfirmation confirmada`
  nas duas** (o sinal que NUNCA apareceu na v1) → SNS→SQS agora realmente confirmado.
  Root cause do bloqueio: a confirmação SNS→SQS nunca completou nas filas v1.
  Novos IDs: sp-traffic `amzn1.fead.cs1.hDgX551EvJ92fgrM989HDg`,
  sp-conversion `amzn1.fead.cs1.tdNa_Mp3BTmfDMqoMahCDA`.
- **1ª entrega esperada:** ~04:00–05:00 UTC (horas cheias após ativação 02:34).
- **Órfãs (bug de cleanup, NÃO bloqueante):** as 2 subscriptions v1
  (`pKB_...`, `oPs...`) seguem ACTIVE apontando pras filas v1 destruídas. O
  `DELETE` retorna 403 SigV4 (`"Invalid key=value pair in Authorization header"`)
  = **a AMS não suporta DELETE**. Correção: arquivar via
  **`PUT /streams/subscriptions/{id}` com body `{"status":"ARCHIVED"}`** (ajustar
  `DeleteSubscription` em `internal/stream/subscriptions.go` — hoje manda DELETE).
  Enquanto não corrige, as órfãs são inofensivas (entrega em fila morta falha do
  lado Amazon; não afeta o v2).

### 15.7 v2 confirmado MAS sem entrega após 3h elegíveis (09/07 ~06:43 UTC)
Monitor v2 rodou 4h (até 06:38 UTC) com ZERO. Já fecharam 3 horas elegíveis
(03-04, 04-05, 05-06 UTC), todas vazias — mesmo com `SubscriptionConfirmation` OK,
KMS descartado e SourceArn correto. **Passou de warm-up.** Duas explicações:
1. 🌙 Madrugada BRT (03-06 UTC = 00-03 BRT) — atividade baixa; possível mas não
   deveria zerar 3h de sp-traffic se houve qualquer impressão.
2. 🔴 Entrega travada do lado Amazon pro profile — confirmação valeu mas o AMS não
   publica → caso de suporte.
**Desempate = CloudWatch** (`NumberOfMessagesSent`, ainda bloqueado por IAM, precisa
root §13.2): 0 nas 3h = Amazon nunca enviou → suporte. >0 = problema nosso.
**Plano:** monitor `b4a384o0t` (30min) cobre até manhã/meio-dia BRT. Se o dado cair
quando o Brasil acordar (~11-14 UTC) = era só madrugada quieta. Se continuar zerado
na manhã BRT = entrega travada → **abrir caso Amazon Ads support** com os
subscriptionIds v2 (`hDgX...`, `tdNa...`). Fallback CSV (§11.3) segue disponível.
...
2. **SNS subscription ativa e confirmada**
   ...
   - v2 `sp-conversion` `amzn1.fead.cs1.tdNa_Mp3BTmfDMqoMahCDA`: `ACTIVE`.
   - v1 traffic/conversion: `ARCHIVED`.
   - O sinal de confirmacao SNS ja apareceu nos logs em §15.5.
...
## 16. Tela Horarios — sincronismo com Robo e ML horario (09/07/2026)

Contexto:
- Usuario alterou bids/multiplicadores no Robo e percebeu que a tela
  `Horarios — Dado Real` ainda mostrava as mesmas recomendacoes.
...
- Logs do modeling-worker confirmam ML horario:
  ...
  - Sempre treinando com `586 celulas campanha×hora`, `90 com pedido`.
  - Sempre gravando `586 predicoes` em `hourly_ml_predictions_v2`.

Conclusao sobre a pagina:
- A tela `Horarios — Dado Real` le
...
## 17. CloudWatch AMS/SQS â€” monitoramento e permissao (09/07/2026)
...
## 19. Amazon Ads Well-Architected — Insights e Recomendacoes (09/07/2026)
...
## 23. HourlyTargetRealV3 — ML keyword/target x hora (09/07/2026)
...
Conclusao:
- A fiação AMS -> SQS v2 -> consumidor -> bronze -> Gold/ML esta funcionando.
...
- `7d-14d`: mais estavel para Sponsored Products/atribuição comum.
...
- AMC existe e esta carregado, mas nao e o mesmo mecanismo do AMS; serve melhor para analises agregadas/atribuição/jornada do que para loop operacional minuto-a-minuto.
...
- Menu `frontend/src/App.jsx` limpo de caracteres quebrados:
  - removidos `â...`, `HorÃ¡rios`, `ConfiguraÃ§Ãµes` etc.
...
- `go test ./internal/query -count=1` passou.
- `npm run build` passou.
- Busca `rg 'Ã|â|ð|�' frontend/src` nao encontrou mojibake visivel restante; sobrou apenas acento valido em `Settings.jsx`.
...
- Arquivo lido: `C:\Users\odine\Downloads\Sponsored_Products_Campanha_relatório (15).csv`.
...
Verificacao:
- CSV baixado da Amazon `Sponsored_Products_Campanha_relatório (15).csv` mostra `8` pedidos no dia:
...
- Arquivo `Sponsored_Products_Campanha_relatório (15).csv` mostra mais uma conversao hoje:
...
- Tráfego esta chegando: 365 linhas campaign com last_traffic_at e 581 linhas target com last_traffic_at.
...
## 39. Diagnóstico robô de bids Mercado — Pincel/Kadukli e falhas em targets (2026-07-10)

Contexto: usuário reportou erro ao atualizar bid da campanha/keyword de Pincel Kaduki/Kabuki. Investigação feita no `mercado-data-app`.

Achados principais:
- A campanha `Kit Kadukli Manga` (`campaign_id=128894883801654`, ad_group `105991687644242`) tem várias keywords relacionadas a Kadukli/Kabuki em `PAUSED` ou travadas por flag de revisão.
- A maioria das entidades da campanha estava com `automation_paused=true` e `cycle_b_enabled=false`, motivo `NEW_CAMPAIGN_REVIEW_REQUIRED`, criado em 2026-06-21. Por isso o robô marcava `SKIPPED_AUTOMATION_PAUSED` e não aplicava as recomendações nelas.
...
- O problema maior das rodadas horárias não era só a campanha Pincel: as execuções do rule `aar-global-hourly-bid-multiplier-no-pause` estavam em `PARTIAL`, avaliando 2225 entidades e falhando cerca de 1985 por hora.
- A causa das 1985 falhas era `TARGET` no endpoint `/sp/targets`: o robô enviava apenas `{targetId,bid}`. A Amazon Ads rejeitou com `400 INVALID_ARGUMENT`, mostrando `expression=null`, `state=null`, `expressionType=null` no `UpdateTargetingClause`.

Correção aplicada no `mercado-data-app`:
...
- Agenda horária (`amazon_ads_bid_schedule_no_pause.go`) agora usa payload completo para TARGET e bloqueia target sem metadado como `BLOCKED_TARGET_METADATA_MISSING` em vez de derrubar o lote inteiro.
...
- `go test ./internal/services -count=1` não concluiu em 120s; foi interrompido por timeout da ferramenta.
- `docker compose build go-backend` + `docker compose up -d --force-recreate go-backend` executado; container `pricing_api` recriado e iniciado às 2026-07-10 13:11 UTC.

Próximo checkpoint:
- A próxima execução horária do robô deve reduzir as falhas de target. Esperado: `failed_count` cair fortemente; se ainda houver falhas, consultar `amazon_ads_automation_execution_items` por `FAILED_AMAZON_API` da nova execução para erro residual.
- Para a campanha `Kit Kadukli Manga`, decidir manualmente se as flags `NEW_CAMPAIGN_REVIEW_REQUIRED` devem ser liberadas. Enquanto `automation_paused=true` / `cycle_b_enabled=false`, o robô continuará pulando essas keywords por desenho, não por erro da Amazon.

Atualizacao §39: apos notar que o primeiro build usou cache em excesso, foi executado build sem cache (docker compose build --no-cache go-backend) e o container pricing_api foi recriado novamente. O go build rodou dentro da imagem, confirmando que a correção entrou no binario em execucao.

### 39.1 Linha do tempo do erro de bids a partir de 00:00 BRT

Evidência adicional:
- Até 2026-07-09 23:00 BRT, o rule `aar-global-hourly-bid-multiplier-no-pause` avaliava 324 entidades e terminava `COMPLETED` com `failed_count=0`.
- Às 2026-07-10 00:00 BRT, a execução `aex-0f49d3c58abb` saltou para 2225 entidades, ficou `PARTIAL` e passou a falhar em 1973 targets.
- O `bootstrap.new_campaign_onboarding` dessa execução mostra `NEW_CAMPAIGN_AUTO_GLOBAL`, `new_bid_entities=2007`, `campaigns_detected=7`, `cycle_b_enabled=true`.
- As 2007 entidades criadas à meia-noite foram principalmente targets: 1966 TARGET e 41 KEYWORD.
- Principais campanhas adicionadas: `SP - All products - product - m19 autopilot - ITG1wbJ7wPUhSzGT` com 1874 targets, `SP - All products - auto - m19 autopilot - m9CiMFKmOjGF/1jM` com 88 targets, `SP - All products - exact - m19 autopilot - vSjnFKqbm+IApSon` com 37 keywords, além de poucos itens em PowerBank-Manual, Localizador, Kit Kadukli Manga e Esponja Gota.
- O sync de targeting que alimentou isso ocorreu por volta de 2026-07-09 23:43-23:44 BRT. O erro só explodiu no apply horário seguinte, à 00:00 BRT.

Conclusão: o início do erro não foi aleatório; foi causado pelo auto-onboarding à meia-noite de campanhas/targets recém-sincronizados. O bug técnico era o payload incompleto para `/sp/targets` (`targetId+bid` sem `expression`, `expressionType`, `state`), corrigido no §39.

### 39.2 Fechamento da validação real do robô de bids (2026-07-10)

Após a correção inicial de payload completo para TARGET, um apply real manual ainda retornou `PARTIAL` porque o aplicador horário enviava 1985 targets em um único PUT `/sp/targets`. A auditoria mostrou `request_count=1985` e erro de validação do array inteiro.

Correção complementar:
- `amazon_ads_bid_schedule_no_pause.go` passou a quebrar mutations em batches (`AMAZON_ADS_BID_SCHEDULE_BATCH_SIZE`, default 50, max 100) com delay configurável (`AMAZON_ADS_BID_SCHEDULE_BATCH_DELAY_MS`, default 700ms).
- Handlers de apply real passaram a usar `context.Background()` com timeout de 20 minutos, para o backend continuar a execução mesmo que a UI/cliente HTTP cancele antes do fim.
- Execução interrompida durante a validação (`aex-3cbaa3e4d880`) foi marcada como `CLIENT_TIMEOUT_CANCELLED` e pendentes como `FAILED_CLIENT_TIMEOUT`, preservando histórico sem deixar `RUNNING` falso.

Validação final:
- Apply real manual `aex-ed9a9adabfd5` iniciou às 2026-07-10 10:57 BRT e terminou às 11:10 BRT.
- Resultado: `COMPLETED`, `evaluated_count=2225`, `applied_count=1832`, `skipped_count=393`, `failed_count=0`, `amazon_attempts=1832`.
- Breakdown final: 1729 TARGET `APPLIED_REAL_CONFIRMED`, 103 KEYWORD `APPLIED_REAL_CONFIRMED`, 263 TARGET já no alvo, demais skips/bloqueios esperados (entidade não enabled, archived, campanha pausada, automação pausada).

Conclusão: o erro de meia-noite foi resolvido 100% na validação real. O robô agora suporta o volume grande de targets das campanhas m19 autopilot sem falhas 400 em massa.

### 39.3 Limpeza da execução das 11h do robô de bids (2026-07-10)

A execução automática das 11h (`aex-a0b89cb960f2`) ficou presa após sobreposição com validações manuais. Não havia escrita ativa recente de BID: último PUT `/sp/targets` observado foi por volta de 11:10 BRT. A execução já estava `STALE_LOCK_EXPIRED`, mas ainda mantinha itens `PENDING_AMAZON_API`.
...
- Cabeçalho atualizado: `status=STALE_LOCK_EXPIRED`, `evaluated_count=2225`, `applied_count=107`, `skipped_count=2118`, `failed_count=0`.

Objetivo: deixar a rodada das 12h entrar limpa, sem herdar pendentes visuais/operacionais da execução sobreposta.

### �39.4 � 2026-07-10 11:40 BRT � Execu��o das 12h antecipada e agenda protegida

Pedido operacional: fechar o `RUNNING` que ainda aparecia no log e puxar a execu��o cr�tica das 12h para antes de sair.

A��es executadas:

- Execu��o `aex-39fb09356c13` (11:28 BRT) marcada como `STALE_LOCK_EXPIRED`; itens pendentes viraram `CANCELLED_STALE_LOCK`.
- Disparada execu��o manual real para `target_hour=12` com confirma��o `APLICAR BIDS POR HORARIO NA AMAZON`.
- Execu��o antecipada criada: `aex-63db8d58089b`.
- Resultado: `COMPLETED`, in�cio `2026-07-10 11:36:01 BRT`, fim `2026-07-10 11:48:23 BRT`, dura��o ~12,4 min.
...
- Auditoria API: `/sp/keywords` HTTP 207 OK em 3 chamadas, `/sp/targets` HTTP 207 OK em 1 chamada com batch m�ximo 25, confirma��es via `/sp/keywords/list` e `/sp/targets/list` OK.
- Para evitar duplicidade da agenda de compra, `amazon_ads_automation_rules.next_run_at` da regra `aar-global-hourly-bid-multiplier-no-pause` foi avan�ado de `2026-07-10 12:00:05 BRT` para `2026-07-10 13:00:05 BRT`.
- Varredura final: zero execu��es `RUNNING` para a regra global.

Conclus�o: a execu��o das 12h foi antecipada com sucesso, sem falhas de Amazon Ads API, e a agenda autom�tica ficou preservada para retomar �s 13h.

### �39.5 � 2026-07-10 14:35 BRT � Corre��o definitiva do scheduler de bids hor�rio

Revis�o ap�s alerta do usu�rio: a execu��o das 13h ainda aparecia `RUNNING` e as execu��es agendadas p�s-corre��o n�o estavam fechando corretamente.

Diagn�stico confirmado:

- `aex-c44338122ccf` (13h scheduler) ficou `RUNNING` sem PUT de bid auditado ap�s a fase inicial; foi marcado como `STALE_LOCK_EXPIRED`.
- `aex-295c795d3a82` (recovery 13h) parou antes do PUT Amazon, com 87 itens `PENDING_AMAZON_API`; foi limpo como `STALE_LOCK_EXPIRED`, `evaluated=194`, `applied=0`, `skipped=194`, `failed=0`. N�o houve escrita parcial de bid nessa execu��o.
- Causa t�cnica: o scheduler usava `context.WithTimeout(..., 3*time.Minute)` enquanto uma rodada real com 2.225 entidades leva ~12-15 min. Quando o contexto expirava, o log podia continuar/retornar, mas as grava��es finais no banco ficavam incompletas, deixando header `RUNNING`.

Corre��es aplicadas no backend:
...
- `amazonAdsBidScheduleApplyNow` ganhou finaliza��o defensiva: se o contexto expirar, cancela itens `PENDING_AMAZON_API` como `CANCELLED_CONTEXT_EXPIRED` e fecha a execu��o como `CLIENT_TIMEOUT_CANCELLED`, evitando stale `RUNNING` silencioso.
- Para scheduler/recovery, `amazonAdsWeekPlanExecuteCurrentHour` passa `skip_already_correct_post_read=true`; assim a rotina n�o gasta tempo confirmando milhares de entidades j� corretas antes dos PUTs. A confirma��o forte continua existindo para o que foi escrito de fato.

Valida��o real:

- Execu��o scheduler `aex-04debba966e6` (14h) finalizou `COMPLETED`.
- In�cio `2026-07-10 14:00:05 BRT`; fim `2026-07-10 14:15:19 BRT`; dura��o ~15,2 min.
...
- Auditoria API da rodada: `/sp/keywords` HTTP 207 OK em 3 chamadas; `/sp/targets` HTTP 207 OK em 1 chamada com batch m�ximo 25; confirma��es `/sp/keywords/list` e `/sp/targets/list` OK.
- Pr�xima execu��o autom�tica: `2026-07-10 15:00:05 BRT`.

Conclus�o: a falha de fechamento das execu��es agendadas era timeout interno do scheduler, n�o rejei��o de bid da Amazon. A rodada real das 14h j� fechou OK ap�s a corre��o de timeout; a otimiza��o de pular post-read pr�vio fica ativa a partir das pr�ximas rodadas scheduler/recovery.

### �40 � 2026-07-10 15:20 BRT � Corre��o da conta AMS conversion: payload existia, parser zerava m�tricas

Alerta do usu�rio: "n�o � poss�vel que AMS n�o retornou nenhum dado de convers�o" e "sua conta est� errada".

Diagn�stico confirmado:

- A conta anterior foi feita no banco errado (`mercado-data-app/pricing_intelligence`) ou olhando campos j� derivados zerados, n�o o payload bruto AMS do `marketcloud`.
- No banco correto (`marketcloud_db`, schema `marketcloud_bronze`), a AMS j� tinha entregue `sp-conversion`.
- Antes da corre��o:
  ...
  - `bronze_ams_hourly_target`: 925 linhas, 11 linhas com `raw_conversion_payload`, mas m�tricas derivadas zeradas.
...
- `internal/stream/consumer.go` agora aceita snake_case e camelCase para convers�es:
...
Resultado correto ap�s backfill:

- `bronze_ams_hourly`: 454 linhas; 11 linhas de convers�o; `orders_1d=8`, `orders_7d=9`, `orders_14d=9`; `sales_1d=401.08`, `sales_7d=446.98`, `sales_14d=446.98`.
- `bronze_ams_hourly_target`: 925 linhas; 11 linhas de convers�o; mesmos totais acima.
- �ltimo `conversion_msg_time`: `2026-07-10 11:00 BRT`.
- �ltimo `last_conversion_at`: `2026-07-10 13:26:30 BRT`.
- `bronze_amazon_ads_hourly` ap�s reconcilia��o: `orders_7d` subiu para `232` e `sales_7d` para `9074.39`.

Conclus�o: AMS retornou convers�es sim. O erro era nosso parser/conta: est�vamos ignorando os nomes reais dos campos (`purchases_*`/`attributed_sales_*` em snake_case). A partir de agora novos payloads entram corretamente, e o hist�rico j� recebido foi corrigido.
...
### §44 - 2026-07-11 14:10 BRT - Loop fechado ML: proposta -> aplicada -> AMS medido -> outcome
...
1. `hourly_real_v2` corrigido contra `Input y contains NaN`.
   ...
   - `cross_val_predict` passou a usar `n_jobs=1` para evitar ruído/erro de joblib no container slim sem `pgrep`.
...
### §45 - 2026-07-11 - Tela Status AMS + ML corrigida e auto-apply ML de campanha preparado
...
#### §45.1 - 2026-07-11 - Correcao final do runtime React `learning is not defined`
...
#### §45.2 - 2026-07-11 - Warning React de key duplicada corrigido
...
- Varredura `rg '�|�|?|�' frontend/src` nao encontrou mojibake restante no frontend.
...
### §46 - 2026-07-12 11:45 BRT - Status AMS confirmado e auto-apply ML verificado
...
- A unica recomendacao BID_UP elegivel pela view no momento era `Localizador Automática`, hora 14, `confidence=LOW`, `rules_still_need_change=1`, `ml_good_hour=true`, `ml_agrees=true`, `0.50 -> 0.80`.
...
### §46.1 - 2026-07-12 11:58 BRT - Correção da tela ML full-auto por campanha

Problema observado: a tela `Settings > Modeling > ML full-auto 360 por campanha` mostrava poucas campanhas e várias linhas com `sem campaign_id no lake`, apesar de haver muitas campanhas no Ads/lake.

Causa:
- A tela não lista todas as campanhas da Amazon; ela listava principalmente campanhas presentes em `marketcloud_gold.gold_hourly_recommendations_v1`.
- O endpoint tentava resolver `campaign_id` por `marketcloud_bronze.bronze_ams_hourly.campaign_name`, mas essa landing crua pode ficar com `campaign_name` vazio. A resolução correta acontece em `marketcloud_bronze.v_ams_hourly_resolved` / refresh.
- Também havia flags em tenants diferentes: as flags ligadas do usuário atual estão em `d7ec8c23-3f86-4cd1-b4cb-2a753a74c5f9`; uma flag antiga de `Seladora=false` estava em `00000000-0000-0000-0000-000000000001`.

Correção aplicada:
- `internal/query/ml_full_auto.go` passou a montar a lista por união de:
  1. campanhas com recomendação (`gold_hourly_recommendations_v1`),
...
Validação SQL com tenant real:
- A listagem passou a retornar 29 campanhas, incluindo campanhas sem recomendação atual.
- Exemplos agora com ID:
  - `Localizador Automática` -> `179355356411697`, full-auto ligado.
  - `Forma Silicone` -> `140196475614872`, full-auto ligado.
  - `Automática com todos os produtos` -> `24988406314413`, full-auto ligado.
...
- API subiu em `:8090`; logs confirmam também consumidor AMS long-poll nas filas v2.

Nota de versionamento:
- `internal/query/ml_full_auto.go` aparece como untracked no checkout atual, então confirmar antes do commit se esse arquivo deve entrar no versionamento junto com as rotas correspondentes.

### §46.2 - Correção Agenda de BIDs por perfil selecionado (2026-07-12)

**Problema:** na tela `http://localhost:3000/#/amazon/ads/automator/bid-robot/schedules`, ao selecionar uma campanha/grupo/BID, a tela e o apply ainda podiam consultar/aplicar usando o universo inteiro de entidades de BID. Isso deixava a operação lenta e dava a impressão de que o filtro por perfil não estava sendo obedecido.

**Correção aplicada em `mercado-data-app`:**
...
Contexto: usuário reportou cockpit http://localhost:3001 fora do ar e pediu para
atacar `rows_unresolved` da reconciliação AMS.

Incidente operacional achado antes: `marketcloud_db` estava PARADO -> api e
orchestrator em crash-loop -> consumidor não drenava -> ~121 msgs presas na fila
v2. `docker start marketcloud_db` restaurou a cascata; fila drenou p/ 0/0, /health 200.

Cockpit 3001: era o dev server Vite (não é container) que tinha morrido. Subido com
`npm run dev -- --port 3001 --host` (node_modules OK; vite.config não fixa porta,
por isso o `--port 3001` explícito). HTTP 200 confirmado.

rows_unresolved (era 227, de 5 campaign_ids):
- Causa: 4 campanhas "m19 autopilot" do parceiro + 1 "Localizador Ataque Concorrente"
  chegam no AMS mas NÃO estão em bronze_swarm_campaign_metrics/bid_schedule (sem
  métrica consolidada nem agenda). Elas TÊM nome em swarm_src.amazon_ads_campaigns_daily.
- Fix (migration 057): tabela local `bronze_swarm_campaign_names` (mapa amplo
  ...
  Resultado: 227 -> 0.
- Durável (migration 058): `refresh_swarm_campaign_names()` virou UPSERT (não
  TRUNCATE — fdw piscar não apaga o mapa) e `refresh_ams_to_hourly()` auto-refresha
  o mapa se >1h stale, dentro de bloco EXCEPTION (fdw fora não derruba a
  reconciliação). Testado forçando stale: refresha sozinho, unresolved=0.

Achado secundário (NÃO tratado ainda): `bronze_ams_hourly` tem impressões
NEGATIVAS em algumas campanhas (ex.: -59, -39) — restatement/delta do AMS. Pode
sujar agregados do Gold/ML. Investigar: o consumidor deve estar somando delta em
vez de last-write-wins, OU o AMS manda valor absoluto negativo. Próximo alvo.

## 41. Resiliência + integridade AMS (2026-07-13) — 2 frentes atacadas

### 41.1 Resiliência (FEITO)
Causa do incidente de hoje (stack caída): `postgres`/`redis` SEM `restart:` no
compose; api/orchestrator/etc em `on-failure` (não cobre reboot/daemon-restart).
...
Pendente (não feito): ALERTA (fila>N / DLQ>0 / db-down) — hoje não há notificação;
recomendado um watchdog/healthcheck externo. Restart resolve o "ficou caído calado".

### 41.2 Integridade AMS — DESCOBERTA GRAVE (interim feito, fix real pendente)
Ao atacar as impressões negativas, medição revelou problema ESTRUTURAL:
- `bronze_ams_hourly`: 1186 células, **544 (46%) com impressão NEGATIVA**, SUM
  total = **506 impressões** em 30 dias. Absurdamente baixo -> os valores são
  **DELTAS/restatement (ou campo errado do payload, §10.6 nunca validado), NÃO
  totais absolutos**. O consumidor faz last-write-wins -> guarda o último delta
  (freq. negativo) em vez do total acumulado.
- Impacto no ML: a reconciliação faz UPSERT (overwrite) em `bronze_amazon_ads_hourly`
  (a fonte do ML). Datas antigas ainda têm o CSV bom (148k impr), mas as **datas
  recentes (pós-CSV) são 100% AMS undercounted** (células com 1-3 impressões) — e
  é o dado mais fresco/relevante pro ML. Pior: **ML_AUTO_APPLY_DRY_RUN=false** — o
  ML aplica bid real; agir sobre dado undercounted é risco.
- INTERIM feito (migration 059): clamp `GREATEST(0,...)` na `v_ams_hourly_resolved`
  -> 0 negativo chega no Gold/ML. Mas clamp NÃO recupera o total (undercount segue).
- FIX REAL (pendente, precisa de cuidado):
  1) CAPTURAR um payload AMS cru (log de 1 mensagem no consumer.go) pra decidir
     definitivamente: delta vs absoluto, e nomes reais dos campos (§10.6).
  2) Se delta: consumidor deve SOMAR por (campaign_id, hora, data) COM idempotência
     por AMS `idempotencyId` (SQS é at-least-once -> somar sem dedup DUPLICA).
     Se campo errado: remapear em consumer.go.
  3) Histórico já ingerido pode ser irrecuperável (deltas antigos foram
     sobrescritos) -> pode precisar reprocessar/re-subscrever.
- RECOMENDAÇÃO até o fix: considerar (a) não deixar o AMS sobrescrever o CSV bom,
  ou (b) o ML continuar priorizando `bronze_amazon_ads_hourly` de origem reporting/CSV
  pra volume, tratando AMS como sinal a validar. Rever com urgência dado o auto-apply.

## 42. Payload AMS cru capturado — diagnóstico DEFINITIVO (2026-07-13)

Capturado via log gated `STREAM_DEBUG_RAW` (consumer.go), já DESLIGADO. Exemplo real:
```json
...
VEREDITO (4 achados):
1. **Nomes de campo CORRETOS** — impressions/clicks/cost/campaign_id batem com o
   consumidor. §10.6 resolvido: NÃO é campo errado.
2. **Valores são DELTA/restatement** — `impressions:-3` é valor real do payload
   (correção). AMS sp-traffic manda incrementos, não absoluto. Nosso bug é
   AGREGAÇÃO (last-write-wins), não leitura.
3. **`idempotency_id` presente** — chave pra dedup seguro (SQS at-least-once).
4. **Grão é KEYWORD×hora×placement, NÃO campanha×hora** — traz keyword_id,
   keyword_text, match_type, placement, ad_id. Prova: campanha 110298784016344
   tem **63 registros keyword** numa única (campanha,dia,hora); o `bronze_ams_hourly`
   (grão campanha, last-write-wins) guarda **1 de 63** -> ~98% descartado. Explica
   o SUM(impressions)=506 absurdo.

CONSEQUÊNCIAS:
- Campo errado: NÃO. Delta: SIM. Colapso de grão: SIM (o pior).
- `bronze_ams_hourly` (campanha) está catastroficamente subcontado.
- `bronze_ams_hourly_target` (keyword) TEM os 63 registros — grão certo — mas por
  keyword ainda faz last-write-wins nos deltas (guarda último delta, não a soma).
- **BÔNUS: Fase 6 (keyword×hora real) está DESBLOQUEADA** — o dado keyword vem no payload.

FIX CORRETO (a fazer):
1. Landing dedup por `idempotency_id` (UNIQUE, ON CONFLICT DO NOTHING) mantendo
   grão keyword/placement + valor do delta.
2. Agregados por SOMA: keyword×hora = SUM(deltas deduped); campanha×hora = SUM(keywords).
3. Reconciliação p/ bronze_amazon_ads_hourly passa a ler o agregado SUM (não overwrite de 1 keyword).
4. Fuso: time_window_start já vem em -03:00 (America/Sao_Paulo) — extração de hora OK.

URGÊNCIA: ML_AUTO_APPLY_DRY_RUN=false aplica bid sobre o campanha-grão subcontado.
Até o fix: recomendado pausar auto-apply OU pausar o overwrite AMS->bronze_amazon_ads_hourly.
Histórico já colapsado é irrecuperável no grão campanha; keyword-landing tem mais dado.
...
- consumer.go: (a) DEDUP por idempotency_id no topo de upsertRecord (pula se já
  visto — seguro p/ at-least-once do SQS); (b) traffic passou de OVERWRITE para
  ACUMULAR (+=) em bronze_ams_hourly (campanha) e bronze_ams_hourly_target (keyword).
  Conversão segue last-write-wins (payload sp-conversion não capturado; 0 por delay).
- Fuso confirmado OK (dateHour parseia -03:00 -> America/Sao_Paulo).

Estado logo após deploy: dedup funcionando (seen_events crescendo, sem erro);
acumulação correta por design mas ainda não visível (só ~7 msgs no 1o minuto —
o lote cheio de ~centenas cai no topo da hora).

RESSALVA HONESTA: histórico já colapsado NÃO recupera (deltas originais já
consumidos pré-fix não reentregam). Só horas FRESCAS (após o deploy) acumulam o
total completo; últimos ~14d só recebem restatements novos (parcial). Datas antigas
do ML seguem no CSV bom (intactas).

Verificação pendente: no próximo lote fresco (~05:00-06:00 UTC) confirmar que
campanha×hora mostra total REALISTA (dezenas/centenas de impressões somando as
keywords), não mais 1-3. Monitorado.

PENDENTE relacionado: capturar payload sp-conversion quando conversões fluírem
p/ decidir delta-vs-absoluto (hoje conversão = overwrite, assumindo absoluto).

### 43.1 VERIFICAÇÃO da acumulação (2026-07-13 ~07:39 UTC)
Prova por data em bronze_ams_hourly:
- **2026-07-13 (dia fresco pós-fix): 24 células, 0 negativas, 24 positivas, soma +121** ✅
  -> dedup+acumular CONFIRMADOS: dado fresco soma correto.
- 2026-07-12 e anteriores: negativas (transição — só recebem restatements-correção
  agora; deltas originais consumidos pré-fix, irrecuperáveis). Clamp (059) neutraliza.
Conclusão: fix validado no dado FRESCO. Histórico da janela é perda esperada.
Volume cresce com o tráfego diurno BR (sempre positivo agora). D-14 empurra o
clamp-0 das datas passadas -> reforça necessidade da camada canônica (tarefa #16)
pra AMS não sobrescrever fonte melhor com o zero-transição.

## 44. Camada canônica gold_hourly_signal_unified IMPLEMENTADA (2026-07-13, tarefa #16)

migration 061: duas views em marketcloud_gold:
- `gold_hourly_signal_unified` — por célula (campanha×hora×dia): métricas +
  ...
  (bool), `signal_note` (aviso honesto). Fonte de verdade com proveniência.
- `gold_hourly_signal_mature` — só conversão CONFIÁVEL (>=7d atribuídos).

Validado (2026-07-13): REPORTING/MATURE = 7864 células, 215 pedidos, roas 4.37 (bom);
AMS_STREAM/IMMATURE = 37 células, 0 pedidos, roas 0 -> corretamente flagadas
"NAO ler 0 como ruim". Resolve o risco #1: célula fresca com roas=0 NÃO é hora
ruim, é conversão imatura — sem a flag, o Gold recomendaria CUT errado.

ADOÇÃO (próximos passos, NÃO feitos — mexem no pipeline auto-apply, decisão do dono):
1. ML: treinar alvo de conversão (has_order/roas) em `gold_hourly_signal_mature`
   (exclui frescas imaturas) — evita aprender "hora fresca = sem pedido". Traffic
   features seguem de todas as células. É o de MAIOR valor dado o auto-apply ligado.
2. D-14 (ams_hourly_refresh.go): não sobrescrever célula MATURE com IMMATURE —
   condicionar o ON CONFLICT à maturidade (para de pisar no reporting bom).
3. Gold/UI: expor traffic_source + conversion_maturity pra não interpretar errado.

### 44.1 Adoção da camada canônica — 3 consumidores plugados (2026-07-13)
Os 3 passos de adoção do §44 IMPLEMENTADOS e verificados:
1. **ML treina conversão no maduro** — `marketcloud_ml_worker_hourly_real_v2.py` load()
   agora lê `gold_hourly_signal_unified`: features de tráfego de TODAS as células,
   alvo (orders/sales/roas) só de `conversion_trustworthy`. Resultado MELHOROU:
   AUC 0.961 (era 0.956), ROAS MAE 1.034 (era 1.305). modeling-worker rebuildado.
2. **D-14 respeita maturidade** — `ams_hourly_refresh.go`: ON CONFLICT agora só
   sobrescreve conversão quando o AMS traz conversão (orders/sales>0); tráfego
   sempre atualiza. Não zera mais conversão madura do CSV com AMS vazio.
   orchestrator rebuildado; rodou limpo (104 upserted, 0 unresolved).
3. **UI mostra fonte/maturidade** — endpoint `/gold/hourly-real` expõe
   `conversion_maturity` (MATURE/MIXED/IMMATURE) + `traffic_source` (subqueries
   correlacionadas sobre a canônica); `HorariosReais.jsx` mostra badge "Fonte".
   Verificado no browser: badge "misto"/"report" com tooltip explicativo.
Camada canônica agora GOVERNA ML, D-14 e UI — não é mais só uma view.

## 45. CSV 08-13 com conversões registrado + refino de maturidade (2026-07-13)

AMS: conversão AINDA 0 (orders_ams/sales_ams NULL em bronze_ams_hourly) — só tráfego.
Por isso o dono trouxe o CSV do console (relatório 17) completando 08-13 COM conversão.

- Ingerido `Sponsored_Products_Campanha_relatório (17).csv` (1501 linhas, 08-13) em
  bronze_amazon_ads_hourly via staging + DISTINCT ON + upsert. Trouxe ~47 pedidos /
  ~R$2.000 nesses dias (antes eram células AMS: ~10 impr/dia, 0 conversão).
- migration 062: refino da camada canônica — `conversion_trustworthy` agora =
  (data madura >=7d) OU (tem conversão real >0). Motivo: a maturidade por data é
  proxy pro AMS (leva 7d); o "pedidos de 7 dias" do CSV já vem atribuído e é usável
  na hora. Sem isso, o ML EXCLUIRIA as conversões recém-carregadas de 08-13.
  Resultado: conversão confiável subiu de 215 -> 266 pedidos.
- D-14 (ams_hourly_refresh.go): tráfego agora usa GREATEST(existente, AMS) — o AMS
  parcial não degrada mais o CSV completo; e acompanha o AMS acumulando nas datas
  frescas. Conversão já era preservada (não sobrescreve com AMS vazio). orchestrator
  rebuildado.
- ML re-treinado: 88 -> 99 células com pedido; Conversão AUC 0.961; ROAS r2 0.15 ->
  0.34 (dobrou a variância explicada). Combustível de conversão renovado.
